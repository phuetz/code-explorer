# Installation de Code Explorer

Derniere mise a jour: 2026-05-07

Ce guide donne le chemin le plus simple pour installer, configurer et lancer Code Explorer en local. Les exemples Windows utilisent PowerShell depuis la racine du depot.
...
```powershell
cd D:\projects\code-explorer-from-c
```
...
- `config-chatgpt.cmd` ecrit `C:\Users\<vous>\.codeexplorer\chat-config.json`.
- `login-chatgpt.cmd` ouvre le navigateur, demande l'authentification ChatGPT et sauvegarde les tokens dans `C:\Users\<vous>\.codeexplorer\auth\openai.json`.
...
Code Explorer doit analyser un projet avant que le chat, les recherches ou les graphes puissent repondre correctement.

```powershell
.\code-explorer.cmd analyze -Repo D:\chemin\vers\mon-projet
```

Exemple avec Sample:

```powershell
.\code-explorer.cmd analyze -Repo D:\projects\sample-app
```

Verifier les depots indexes:

```powershell
.\code-explorer.cmd list
```
...
```powershell
.\code-explorer.cmd chat
```

Par defaut, le script lance:

- backend HTTP Code Explorer: `http://127.0.0.1:3010`
...
```powershell
.\code-explorer.cmd chat -BackendPort 3011 -ChatPort 5177
```
...
Cette navigation utilise uniquement les depots indexes par Code Explorer. Si le panneau est vide, lancez d'abord:
...
```powershell
.\code-explorer.cmd docs -Repo D:\chemin\vers\mon-projet
```

Generation enrichie par LLM:

```powershell
.\code-explorer.cmd docs -Repo D:\chemin\vers\mon-projet -Enrich
```

Produire sans ouvrir le navigateur:

```powershell
.\code-explorer.cmd docs -Repo D:\chemin\vers\mon-projet -NoBrowser
```
...
Code Explorer sait lire un manifeste compatible DeepWiki dans le repo analyse:
...
- dans la CLI, `code-explorer generate pdf --input <markdown>` produit un PDF A4 via Chromium/Playwright et attend les diagrammes Mermaid, les polices et les images avant capture.

Les exports nettoient les caracteres invisibles qui peuvent perturber les moteurs PDF. Si un diagramme Mermaid ne peut pas etre rendu, Code Explorer conserve la source du diagramme dans le document au lieu de produire un bloc vide.
...
.\code-explorer.cmd list
.\code-explorer.cmd analyze -Repo D:\chemin\vers\mon-projet
.\code-explorer.cmd ask -Question "Resume l'architecture du projet en 5 points"
.\code-explorer.cmd docs -Repo D:\chemin\vers\mon-projet
.\code-explorer.cmd chat -RestartBackend -RestartChat
```
...
```powershell
Remove-Item "$env:USERPROFILE\.codeexplorer\auth\openai.json"
.\login-chatgpt.cmd
```
...
```text
Trace le flux d'execution complet d'une creation de lettre en masse avec un diagramme Mermaid flowchart TD, puis liste les sources.
```
