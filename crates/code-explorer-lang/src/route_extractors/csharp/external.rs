...
pub fn extract_external_service_calls(source: &str) -> Vec<ExternalServiceCall> {
    ...
    for (line_idx, line) in source.lines().enumerate() {
        ...
        // --- WebAPI client instantiation: new AcmeClient(httpClient) ---
        ...
        // --- WCF service calls: new LegacySvc(), new IntegrationWS() ---
...
mod tests {
    ...
    fn test_extract_external_service_calls() {
        let source = r#"
var customerClient = new AcmeClient(httpClient);
var result = customerClient.CustomersGetAsync(orgId, id, token).GetAwaiter().GetResult();
var partnerClient = new PartnerClient(httpClient);
var members = partnerClient.MembersGetAsync(id).GetAwaiter().GetResult();
"#;
        ...
        assert!(calls.iter().any(|c| c.client_class == "AcmeClient"));
        assert!(calls.iter().any(|c| c.client_class == "PartnerClient"));
    }

    #[test]
    fn test_extract_wcf_service_calls() {
        let source = r#"
var svc = new LegacySvc();
var ws = new IntegrationWS();
"#;
        let calls = extract_external_service_calls(source);
        assert_eq!(calls.len(), 2);
        assert!(calls
            .iter()
            .any(|c| c.client_class == "LegacySvc" && c.service_type == "WCF"));
        assert!(calls
            .iter()
            .any(|c| c.client_class == "IntegrationWS" && c.service_type == "WCF"));
    }
