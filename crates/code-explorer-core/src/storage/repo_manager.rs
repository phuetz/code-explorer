...
/// Layout version of an on-disk index.
///
/// Stamped into `meta.json` by [`save_meta`] so `code-explorer doctor` can tell
/// "written by this build" from "written by an older build that predates the
/// stamp" (`schema_version: None`) or by a newer one.
...
pub struct RepoMeta {
    ...
    /// Index layout version, stamped by [`save_meta`]. `None` for an index
...
pub struct RepoStats {
    ...
    /// Prose documents indexed (Markdown, text, reStructuredText). `None` on
    /// an index written before prose indexing existed, `Some(0)` when prose
...
pub fn read_registry() -> Result<Vec<RegistryEntry>> {
    read_registry_from(&get_global_registry_path())
}

/// Read a registry file from an explicit path. Returns an empty vec when the
/// file does not exist yet.
///
/// Long-lived readers (the MCP server) need to re-read a specific registry
/// file to pick up repositories indexed after they started, and tests need to
/// point at a sandbox registry without mutating process-global environment
/// variables — both go through this function.
pub fn read_registry_from(path: &Path) -> Result<Vec<RegistryEntry>> {
...
pub fn save_meta(storage_path: &Path, meta: &RepoMeta) -> Result<()> {
    ...
    let meta_path = storage_path.join("meta.json");
    // The writer is always the current build, so stamp the current layout
    // version rather than trusting whatever the caller left in the struct.
    let stamped = RepoMeta {
