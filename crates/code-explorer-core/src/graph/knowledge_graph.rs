...
///
/// Stores nodes and relationships in BTreeMaps for stable iteration and O(log n) lookup.
...
impl KnowledgeGraph {
    ...
    // Retained for API compatibility; ordered maps do not reserve capacity.
    pub fn with_capacity(_nodes: usize, _relationships: usize) -> Self {
        Self::default()
    }
    ...
    pub fn add_node(&mut self, node: GraphNode) {
        ...
        let ids = self.file_index.entry(file_path).or_default();
    ...
    /// Remove `Folder` nodes that no longer contain anything.
    ///
    /// `remove_nodes_by_file` deletes the nodes of a file and the edges that
    /// touch them, but a folder is not a file: after a deletion or a rename
    /// its `Folder` node survives with no children, and every consumer
    /// (`context`, diagrams, the desktop tree) keeps showing a directory that
    /// no longer exists. Prunes bottom-up so a chain of emptied folders goes
    /// in one call. Returns the number of folders removed.
    pub fn remove_empty_folders(&mut self) -> usize {
        use super::types::NodeLabel;
        let mut removed = 0usize;
        loop {
            let mut has_child: std::collections::HashSet<&str> = std::collections::HashSet::new();
            for rel in self.relationships.values() {
                if rel.rel_type == super::types::RelationshipType::Contains {
                    has_child.insert(rel.source_id.as_str());
                }
            }
    ...
    /// Remove a relationship by id, returning it if present.
    pub fn remove_relationship(&mut self, id: &str) -> Option<GraphRelationship> {
...
mod tests {
    ...
    /// A deletion or a rename must not leave a directory behind.
    #[test]
    fn test_remove_empty_folders_prunes_the_whole_chain() {
        let mut graph = KnowledgeGraph::new();
        graph.add_node(make_node("Folder:docs", NodeLabel::Folder, "docs"));
        ...
        graph.add_node(make_node("Folder:src", NodeLabel::Folder, "src"));
        graph.add_node(make_node("File:src/a.rs", NodeLabel::File, "src/a.rs"));
        graph.add_relationship(make_rel(
            "c1",
            "Folder:docs",
            "Folder:docs/guide",
        ...
        assert_eq!(graph.remove_empty_folders(), 0);

        graph.remove_nodes_by_file("docs/guide/intro.md");
        // Both `docs/guide` and `docs` are now empty and go in one pass.
        assert_eq!(graph.remove_empty_folders(), 2);

        assert!(graph.get_node("Folder:docs").is_none());
        assert!(graph.get_node("Folder:docs/guide").is_none());
        // The folder that still holds a file is untouched.
        assert!(graph.get_node("Folder:src").is_some());
        assert!(graph.get_node("File:src/a.rs").is_some());
    }
