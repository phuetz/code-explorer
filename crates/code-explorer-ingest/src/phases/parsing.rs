...
/// Content-addressed, pre-resolution parse results. Never cache enriched nodes:
/// their properties and incident edges depend on the entire repository.
#[derive(serde::Serialize, serde::Deserialize)]
struct ParseCache {
    executable: String,
    files: std::collections::HashMap<String, (String, FileParsed)>,
}

pub(crate) fn parse_files_cached(
    graph: &mut KnowledgeGraph,
    files: &[FileEntry],
    ...
    let executable = std::env::current_exe()
        ...
        .and_then(|m| Some(format!("{}:{:?}", m.len(), m.modified().ok()?)))
        .unwrap_or_default();
    let path = storage.join("parse-cache.bin");
    let mut fallback = None;
    let mut cached = if incremental && !executable.is_empty() {
        match std::fs::read(&path)
            .ok()
            .and_then(|bytes| {
                let text = std::str::from_utf8(&bytes).ok()?;
                let (digest, payload) = text.split_once('\n')?;
                if crate::manifest::compute_hash_from_content(payload) != digest {
                    return None;
                }
                serde_json::from_str::<ParseCache>(payload).ok()
            })
            .filter(|cache| cache.executable == executable)
        {
            Some(cache) => cache.files,
            None => {
                fallback = Some(
                    "parse cache missing, incompatible or unreadable; full refresh".to_string(),
                );
                Default::default()
            }
        }
    ...
    if let Some(reason) = &fallback {
        eprintln!("Incremental fallback: {reason}");
    }
    ...
    for file in files {
        ...
        let hash = crate::manifest::compute_hash_from_content(&file.content);
        match cached.remove(&file.path) {
            Some((old_hash, parsed)) if old_hash == hash => {
                reused.insert(file.path.clone(), (hash, parsed));
            }
            _ => pending.push((file, lang, hash)),
        }
    }
    let parsed_files = pending.len();
    let parsed: Vec<_> = pending
        .into_par_iter()
        .map(|(file, lang, hash)| (file.path.clone(), (hash, parse_single_file(file, lang))))
        .collect();
    reused.extend(parsed);
    let cache = ParseCache {
        executable,
        files: reused,
    };
    // Cache publication is independent of snapshot publication: entries are pure
    // parse results keyed by content, so interrupted graph saves cannot make them stale.
    let save = || -> std::io::Result<()> {
        ...
        writeln!(
            out,
            "{}",
            crate::manifest::compute_hash_from_content(&payload)
        )?;
    ...
    if let Err(error) = save() {
        tracing::warn!(%error, "Could not save parse cache; next run may need full parsing");
    }
    let mut extracted = ExtractedData::default();
    let mut results = cache.files;
    // Keep exactly the same merge order as full parsing, including collisions.
    for file in files {
        if let Some((_, result)) = results.remove(&file.path) {
            for node in result.nodes {
                graph.add_node(node);
            }
            for rel in result.relationships {
    ...
    Ok((extracted, parsed_files, fallback))
}
...
"#
    ...
    fn test_build_symbol_table() {
        ...
        let global = &table.lookup_global("handleLogin").unwrap()[0];
        let local = &table.lookup_in_file("src/main.ts", "handleLogin").unwrap()[0];
        assert_eq!(global.owner_id.as_deref(), Some("Class:src/main.ts:Login"));
        assert_eq!(local.owner_id, global.owner_id);
        assert!(std::sync::Arc::ptr_eq(global, local));
    }
}
