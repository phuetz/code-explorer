...
// Pattern 2: Constructor DI params like "public Foo(ILetterService letterService)"
static CLASS_RE: Lazy<Regex> = Lazy::new(|| {
    Regex::new(r"(?:public|internal|private)?\s*(?:partial\s+)?class\s+(\w+)")
        .expect("CLASS_RE must compile")
});

// Pattern 3: C# 'using' statement: using (var x = new LetterService(...))
...
fn build_field_type_map(
    ...
    for file in file_entries {
        ...
        // Extract 'using' pattern: using (var svc = new LetterService(...))
...
pub fn resolve_calls(
    ...
    for call in &extracted.calls {
        ...
        // e.g., BulkLetterRules.LetterGenerationProcess(...)
        if call.file_path.ends_with(".cs") {
            if let Some(ref receiver) = call.receiver_name {
