...
pub struct PipelineResult {
    ...
    /// What the prose phase added (all zeros when prose was not indexed).
    pub doc_stats: phases::docs::DocStats,
...
pub struct PipelineOptions {
    ...
    /// Index prose files (Markdown, text, reStructuredText).
    /// `None` decides automatically: prose wins on a repository that is
    /// mostly prose (see [`phases::docs::should_index_docs`]).
    pub include_docs: Option<bool>,
    /// Directories to keep out of the walk. `None` resolves the repository's
    /// own rules (defaults + `.codeexplorer/config` + `code-explorer.toml`).
    pub exclusions: Option<code_explorer_core::config::exclusions::ExclusionRules>,
}
...
pub async fn run_pipeline(
    ...
    let exclusions = options.exclusions.clone().unwrap_or_else(|| {
        code_explorer_core::config::exclusions::ExclusionRules::for_repo(repo_path)
    });
    let file_entries = phases::structure::walk_repository_with(repo_path, &exclusions)?;
    ...
    // Phase 1c: Prose documents (Markdown / text / reStructuredText).
    //
    // The code walker keeps only files with a supported programming language,
    // so a book or a documentation site used to index to almost nothing. Walk
    // prose separately and, when it dominates the repository (or the caller
    // asked for it), give it File + Section nodes and internal-link edges.
    let phase_start = Instant::now();
    let mut doc_entries = phases::docs::walk_documents_with(repo_path, &exclusions)?;
    let index_docs = options
        .include_docs
        .unwrap_or_else(|| phases::docs::should_index_docs(total_files, doc_entries.len()));
    ...
    // Run after resolution so invalidation includes newly created direct
    ...
    for stats in per_file {
        ...
        api_stats.fastapi_flask += stats.fastapi_flask;
        api_stats.spring += stats.spring;
        api_stats.go += stats.go;
        api_stats.php += stats.php;
        api_stats.ruby += stats.ruby;
    }
    let duration = phase_start.elapsed();
    tracing::info!(
        phase = "api_surface",
    ...
    let phase_start = Instant::now();
    local_enrichment.save(&graph);
    phase_timings.push(PhaseTiming {
        name: "local_enrichment_cache".into(),
        duration_ms: phase_start.elapsed().as_millis() as u64,
    });
    ...
    Ok(PipelineResult {
        ...
        local_enrichments: local_enrichment.counts,
