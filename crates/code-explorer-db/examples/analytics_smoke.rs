...
//!
//! Usage: cargo run --release -p code-explorer-db --example analytics_smoke -- <snap_path> [repo_root]
use std::env;
use std::path::PathBuf;

use code_explorer_db::analytics::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if args.len() < 2 {
        eprintln!("Usage: analytics_smoke <snap_path> [repo_root]");
        std::process::exit(2);
    }
