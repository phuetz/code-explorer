...
pub async fn start_mcp_server(mut backend: LocalBackend) -> crate::error::Result<()> {
    ...
    info!("Code Explorer MCP server starting on stdio");
...
mod tests {
    ...
    /// Every tool failure must arrive with a machine-readable code and a hint,
    /// not only a sentence of English.
    #[tokio::test]
    async fn test_tool_errors_carry_a_structured_payload() {
        let mut backend = LocalBackend::new();
        ...
        .await;
        let error = unknown.error.expect("unknown tool must fail");
        let data = error.data.expect("error must carry data");
        assert_eq!(data["code"], "UNKNOWN_TOOL");
        assert_eq!(data["tool"], "no_such_tool");
        assert!(data["hint"].as_str().unwrap().contains("tools/list"));

        let bad_args = handle_request(
            &make_request(
                "tools/call",
                json!({"name": "context", "arguments": {"repo": "whatever"}}),
            ),
            &mut backend,
        )
        .await;
        let error = bad_args.error.expect("missing 'name' must fail");
        let data = error.data.expect("error must carry data");
        assert_eq!(data["code"], "INVALID_ARGUMENTS");
        assert_eq!(data["tool"], "context");
    ...
    async fn test_handle_resource_read() {
        let mut backend = LocalBackend::new();
        let req = make_request("resources/read", json!({"uri": "code-explorer://version"}));
        let resp = handle_request(&req, &mut backend).await;
        assert!(resp.result.is_some());
    }
