...
use code_explorer_core::impact::{analyze_impact, ImpactNode};
use code_explorer_core::storage::repo_manager::{self, registry_entry_id, RegistryEntry};
use code_explorer_db::inmemory::cypher::GraphIndexes;
use code_explorer_db::inmemory::fts::{fts_result_to_json, FtsIndex, FtsResult};
use code_explorer_db::pool::ConnectionPool;
use code_explorer_db::query;
use code_explorer_git::coupling::CouplingError;
use code_explorer_git::hotspots::HotspotError;
use code_explorer_git::ownership::OwnershipError;
use code_explorer_search::bm25::BM25SearchResult;
use code_explorer_search::embeddings::{
    generate_embeddings, load_embeddings, search_semantic, EmbeddingConfig,
};
use code_explorer_search::hybrid;
...
pub struct LocalBackend {
    ...
    /// Registry file this backend mirrors. Overridable so tests (and embedders)
    /// can point at a sandbox registry without touching process-wide env vars.
    registry_path: PathBuf,
    /// Fingerprint of `registry_path` at the time the registry was loaded.
    /// `None` means "not loaded yet, or the file did not exist".
    registry_fingerprint: Option<FileFingerprint>,
    /// Whether this backend mirrors the registry file. Set by `init()`.
    /// A backend whose registry was populated programmatically (embedders,
    /// tests) owns its list and is never overwritten from disk.
    follow_registry_file: bool,
    ...
    /// Cheap disk fingerprints used to detect snapshots replaced by re-indexing.
    snapshot_fingerprints: HashMap<PathBuf, FileFingerprint>,
...
/// Cheap disk fingerprint (length + mtime) used to detect a file replaced
/// behind a long-lived cache: graph snapshots, and the global registry.
...
fn git_history_required_error(tool: &str) -> McpError {
    McpError::InvalidArguments {
        tool: tool.into(),
        reason: "Git analytics require an indexed path inside a Git working tree. Choose a Git-backed project or re-run `code-explorer analyze` from the repository checkout.".into(),
    }
}
...
/// Normalized key for a path that exists on disk, resolved through `.`, `..`,
/// symlinks and trailing separators. `None` when the path cannot be resolved
/// (it does not exist, or is not readable).
fn canonical_lookup_key(raw: &str) -> Option<String> {
    let canonical = std::path::Path::new(raw).canonicalize().ok()?;
    Some(normalize_repo_lookup_key(&canonical.display().to_string()))
}

/// Whether a registry entry still has a readable index on disk.
fn index_is_present(entry: &RegistryEntry) -> bool {
    let storage = std::path::Path::new(&entry.storage_path);
    storage.join("meta.json").is_file() && storage.join("graph.bin").is_file()
}

/// Build a registry entry from an index sitting on disk, for a repository the
/// registry does not know about. Returns `None` unless the path is a directory
/// carrying a readable `.codeexplorer/meta.json`.
fn entry_from_disk(raw: &str) -> Option<RegistryEntry> {
    let path = std::path::Path::new(raw).canonicalize().ok()?;
    if !path.is_dir() {
        return None;
    }
    let storage = repo_manager::get_storage_path(&path);
    let meta = repo_manager::load_meta(&storage).ok().flatten()?;
    Some(RegistryEntry {
        name: path
            .file_name()
            .and_then(|n| n.to_str())
...
impl LocalBackend {
    ...
    pub fn new() -> Self {
        Self::with_registry_path(repo_manager::get_global_registry_path())
    }

    /// Create a backend bound to an explicit registry file.
    pub fn with_registry_path(registry_path: PathBuf) -> Self {
        Self {
            ...
            follow_registry_file: false,
            snapshot_cache: HashMap::new(),
            snapshot_fingerprints: HashMap::new(),
            indexes_cache: HashMap::new(),
            fts_cache: HashMap::new(),
        }
    }

    fn invalidate_snapshot_caches(&mut self, key: &std::path::Path) {
        self.snapshot_cache.remove(key);
        self.snapshot_fingerprints.remove(key);
        self.indexes_cache.remove(key);
        self.fts_cache.remove(key);
    }

    /// Load a snapshot from disk, returning a cached `Arc<KnowledgeGraph>` if already loaded.
    ///
    /// Freshness uses file length and modification time so normal lookups do not re-read the
    /// full snapshot. A replacement that preserves both values cannot be detected without a
    /// persisted snapshot generation, which the current snapshot format does not provide.
    fn load_cached_snapshot(&mut self, snap_path: &std::path::Path) -> Result<Arc<KnowledgeGraph>> {
        let key = snap_path.to_path_buf();

        // Retry once when an atomic replacement lands between metadata and snapshot reads.
        for attempt in 0..2 {
            let fingerprint = match FileFingerprint::read(snap_path) {
                Ok(fingerprint) => fingerprint,
                Err(error) => {
                    self.invalidate_snapshot_caches(&key);
                    return Err(McpError::Internal(format!(
                        "Failed to inspect graph snapshot: {error}"
                    )));
                }
            };

            if self.snapshot_fingerprints.get(&key) == Some(&fingerprint) {
            ...
            let graph = match code_explorer_db::snapshot::load_snapshot(snap_path) {
                Ok(graph) => graph,
                Err(error) => {
                    self.invalidate_snapshot_caches(&key);
                    return Err(McpError::Internal(format!(
            ...
            let loaded_fingerprint = match FileFingerprint::read(snap_path) {
                ...
                Err(error) => {
                    self.invalidate_snapshot_caches(&key);
                    return Err(McpError::Internal(format!(
                        "Failed to inspect loaded graph snapshot: {error}"
                    )));
                }
            };
            ...
            if loaded_fingerprint != fingerprint {
                ...
                return Err(McpError::Internal(
                    "Graph snapshot changed repeatedly while it was being loaded".to_string(),
                ));
            }

            let arc = Arc::new(graph);
            self.snapshot_fingerprints.insert(key.clone(), fingerprint);
            self.snapshot_cache.insert(key, Arc::clone(&arc));
            return Ok(arc);
        }

        unreachable!("snapshot load loop always returns")
    }
    ...
    pub fn init(&mut self) -> Result<()> {
        self.follow_registry_file = true;
    ...
    /// Read the registry file and remember its fingerprint.
    fn reload_registry(&mut self) -> Result<()> {
        self.registry =
            repo_manager::read_registry_from(&self.registry_path).map_err(McpError::Core)?;
        self.registry_fingerprint = FileFingerprint::read(&self.registry_path).ok();
        Ok(())
    }

    /// Re-read the registry when the file on disk changed since it was loaded.
    ///
    /// The registry used to be read once, in `init()`. An MCP server lives for
    /// the whole editor/agent session, so every repository indexed after the
    /// server started stayed invisible to it: `context`/`impact` answered
    /// "Repository not found" while the CLI — which never consults the
    /// registry — answered correctly on the very same index (defect D1).
    /// One `stat` per tool call is enough to keep the two surfaces in sync.
    fn refresh_registry_if_changed(&mut self) {
        if !self.follow_registry_file {
            return;
        }
        let current = FileFingerprint::read(&self.registry_path).ok();
        if current == self.registry_fingerprint {
            return;
        }
        match self.reload_registry() {
            Ok(()) => info!(
                "LocalBackend: registry changed on disk, reloaded {} repos",
                self.registry.len()
            ),
            Err(e) => {
                // A half-written or unreadable registry must not break tools
                // that were already working: keep the previous snapshot.
                warn!("LocalBackend: registry reload failed, keeping previous snapshot: {e}");
            }
        }
    }
    ...
    /// Otherwise searches by name or path (case-insensitive), then by
    /// canonical path, then falls back to an index found on disk.
    ///
    /// Returns an owned entry: resolution may synthesise an entry that is not
    /// in the registry, and every caller needs the mutable backend afterwards
    /// to populate caches.
    pub fn resolve_repo(&self, repo: Option<&str>) -> Result<RegistryEntry> {
        match repo {
            None => {
                if self.registry.len() == 1 {
                    Ok(self.registry[0].clone())
                } else if self.registry.is_empty() {
                    Err(McpError::RepoNotFound(
                        "No repositories indexed. Run `code-explorer analyze <path>` first.".into(),
                    ))
                } else {
            ...
            Some(name_or_path) => {
                if let Some(entry) = self.lookup_registry(name_or_path) {
                    return Ok(entry.clone());
                }
                // The argument names a directory that carries its own index but
                // is missing from the registry (registry lost or written under a
                // different CODE_EXPLORER_HOME, index copied with the checkout).
                // The CLI reads such an index without complaining; the MCP
                // server must not be stricter than the CLI on the same data.
                if let Some(entry) = entry_from_disk(name_or_path) {
                    info!(
                        "LocalBackend: '{}' is not in the registry but has an index on disk; using it",
                        name_or_path
                    );
                    return Ok(entry);
                }
                Err(self.repo_not_found(name_or_path))
            }
        }
    }

    /// Match a repo argument against the in-memory registry.
    fn lookup_registry(&self, name_or_path: &str) -> Option<&RegistryEntry> {
        let lookup_key = normalize_repo_lookup_key(name_or_path);
        let matches_key = |e: &RegistryEntry, key: &str| -> bool {
            if registry_entry_id(e) == key {
                return true;
            }
            // Exact name / exact path / suffix on a path-segment boundary.
            // Without the segment guard, "foo" would also match
            // "/repos/myfoo", silently selecting the wrong repo for any name
            // that's a tail substring.
            ...
            !key.contains('/') && path_key.ends_with(&format!("/{}", key))
        };

        if let Some(entry) = self.registry.iter().find(|e| matches_key(e, &lookup_key)) {
            return Some(entry);
        }

        // Second pass on canonical paths: `.`/`..` segments, trailing slashes,
        // symlinked parents and relative arguments all name the same repo but
        // never match the raw string stored in the registry.
        let canonical_key = canonical_lookup_key(name_or_path)?;
        self.registry.iter().find(|e| {
            matches_key(e, &canonical_key)
                || canonical_lookup_key(&e.path).is_some_and(|k| k == canonical_key)
        })
    }

    /// Build the actionable "repository not found" error: what was looked up,
    /// where, and the exact command that fixes it.
    fn repo_not_found(&self, name_or_path: &str) -> McpError {
        let looked_up = canonical_lookup_key(name_or_path)
            .unwrap_or_else(|| normalize_repo_lookup_key(name_or_path));
        let mut message = format!(
            "Repository not found: '{name_or_path}'. Looked up '{looked_up}' in the registry {} ({} indexed repositor{}).",
            self.registry_path.display(),
            self.registry.len(),
            if self.registry.len() == 1 { "y" } else { "ies" }
        );

        let as_path = std::path::Path::new(name_or_path);
        if as_path.is_dir() {
            let storage = repo_manager::get_storage_path(as_path);
            message.push_str(&format!(
                " The directory exists but has no readable index at '{}'.",
                storage.join("meta.json").display()
            ));
        } else {
            message.push_str(" No such directory on this machine.");
        }

        let suggestions = self.closest_repo_names(name_or_path);
        if !suggestions.is_empty() {
            message.push_str(&format!(" Did you mean: {}?", suggestions.join(", ")));
        }

        message.push_str(&format!(
            " Fix: run `code-explorer analyze {name_or_path}` — the MCP server reloads the \
             registry on the next call, no restart needed."
        ));
        McpError::RepoNotFound(message)
    }

    /// Up to three registry names that plausibly match the argument, so the
    /// caller can retry without listing every indexed repository.
    fn closest_repo_names(&self, name_or_path: &str) -> Vec<String> {
        let key = normalize_repo_lookup_key(name_or_path);
        let needle = key.rsplit('/').next().unwrap_or(&key).to_string();
        if needle.is_empty() {
            return Vec::new();
        }
        self.registry
    ...
    pub async fn call_tool(&mut self, name: &str, args: &Value) -> Result<Value> {
        // Pick up repositories indexed since the server started (defect D1).
    ...
    /// List the repositories this server can actually answer about.
    ///
    /// The registry is a record of past `analyze` runs, not a description of
    /// the disk: a deleted index, a `clean`, a removed checkout all leave an
    /// entry behind. Listing those is worse than useless — an agent picks one,
    /// every tool then fails on it. Entries whose index is gone are filtered
    /// out and merely counted.
    fn tool_list_repos(&self) -> Result<Value> {
        ...
        let repos: Vec<Value> = self
            ...
            .iter()
            .filter(|e| {
                let present = index_is_present(e);
                if !present {
                    stale += 1;
                }
                present
        ...
        let mut meta = json!({
            ...
            "counts": { "listed": repos.len(), "staleEntriesHidden": stale }
        });
        if stale > 0 {
            meta["note"] = Value::String(format!(
                "{stale} registry entr{} no longer ha{} an index on disk and {} hidden; \
                 re-run `code-explorer analyze <path>` to bring one back.",
                if stale == 1 { "y" } else { "ies" },
                if stale == 1 { "s" } else { "ve" },
                if stale == 1 { "was" } else { "were" },
            ));
        }
    ...
    async fn tool_impact(&mut self, args: &Value) -> Result<Value> {
        ...
        let graph = self.load_cached_snapshot(&snap_path)?;
        let impact = analyze_impact(graph.as_ref(), target, max_depth)
            .ok_or_else(|| McpError::Internal(format!("Symbol '{}' not found in graph", target)))?;

        let format_rows = |rows: &[ImpactNode]| -> Vec<Value> {
            rows.iter()
                .filter_map(|impact_node| {
                    let node = graph.get_node(&impact_node.id)?;
                    Some(json!({
                        "name": node.properties.name,
                        "id": node.id,
                        "filePath": node.properties.file_path,
                        "label": node.label.as_str(),
                        "depth": impact_node.depth,
                    }))
                })
                .collect()
        };
        ...
        if direction == "upstream" || direction == "both" {
            let callers = format_rows(&impact.upstream.nodes);
        ...
        if direction == "downstream" || direction == "both" {
            let callees = format_rows(&impact.downstream.nodes);
            all_results.push(json!({
        ...
        let enrichment = Self::collect_enrichment_for_node(&graph, &impact.target.id);
        ...
        Ok(json!({
            ...
            "_meta": {
                ...
                "direction": direction,
                "maxDepth": max_depth,
                "matchedTargetId": impact.target.id,
                "counts": {
                    "upstream": impact.upstream.total(),
                    "downstream": impact.downstream.total(),
                    "total": impact.upstream.total() + impact.downstream.total()
                },
                "depthCounts": {
                    "upstream": impact.upstream.depth_counts,
                    "downstream": impact.downstream.depth_counts
                },
                "enrichment": enrichment
            }
        }))
                ...
                return Ok(json!({
                    "content": [{
                        ...
                        "text": serde_json::to_string_pretty(&json!({
                            ...
                            "note": "No graph snapshot available — run `code-explorer analyze` to enable symbol/process mapping.",
    ...
    async fn tool_coupling(&self, args: &Value) -> Result<Value> {
        ...
        let mut couplings =
            match code_explorer_git::coupling::analyze_coupling(repo_path, min_shared, Some(180)) {
    ...
    async fn tool_ownership(&self, args: &Value) -> Result<Value> {
        ...
        let mut ownerships = match code_explorer_git::ownership::analyze_ownership(repo_path) {
    ...
    async fn tool_analyze_execution_trace(&mut self, args: &Value) -> Result<Value> {
        ...
        let steps = code_explorer_core::trace::parse_trace(&trace_content)
            .map_err(|e| McpError::Internal(format!("Failed to parse trace: {}", e)))?;

        let name_to_ids = code_explorer_core::trace::build_name_index(&graph);
        ...
        for step in steps {
            ...
            if let Some(full_method_name) = method_name_opt {
                if let Some(node_id) = code_explorer_core::trace::resolve_method_node(
                    &graph,
                    &name_to_ids,
                    full_method_name,
                ) {
    ...
    async fn tool_search_code(&mut self, args: &Value) -> Result<Value> {
        ...
        // Post-retrieval LLM reranking (opt-in). Requires ~/.codeexplorer/chat-config.json.
    ...
    async fn maybe_hybrid_fuse(
        hybrid_mode: bool,
        query_str: &str,
        graph: &std::sync::Arc<code_explorer_core::graph::KnowledgeGraph>,
        ...
        if !emb_path.exists() || !meta_path.exists() {
            tracing::warn!(
                "hybrid=true but embeddings files missing at {} — run 'code-explorer embed' first; using BM25 order",
    ...
    async fn maybe_rerank_fts(
        ...
        let config = match crate::llm_config::load_llm_config() {
            ...
            None => {
                tracing::warn!(
                    "rerank=true but ~/.codeexplorer/chat-config.json missing; using BM25 order"
                );
                return fts_results;
            }
        };
    ...
    async fn tool_save_memory(&mut self, args: &Value) -> Result<Value> {
        ...
        let memory_dir = if scope == "global" {
            // Global memory: ~/.codeexplorer/memory/
            ...
            // Project memory: .codeexplorer/memory/
            let entry = self.resolve_repo(repo_name)?;
    ...
    async fn tool_find_cycles(&mut self, args: &Value) -> Result<Value> {
        use code_explorer_db::analytics::cycles::{find_cycles, CycleScope};
        let repo_name = args.get("repo").and_then(|v| v.as_str());
        let scope_str = args
            .get("scope")
            .and_then(|v| v.as_str())
    ...
    async fn tool_find_similar_code(&mut self, args: &Value) -> Result<Value> {
        use code_explorer_db::analytics::clones::{find_clones, CloneOptions};
        let repo_name = args.get("repo").and_then(|v| v.as_str());
        let min_tokens = args
            .get("min_tokens")
            .and_then(|v| v.as_u64())
    ...
    async fn tool_get_complexity(&mut self, args: &Value) -> Result<Value> {
        use code_explorer_db::analytics::complexity::{get_complexity, ComplexityOptions};
        let repo_name = args.get("repo").and_then(|v| v.as_str());
    ...
    // ─── SFD doc-authoring workflow (P1.1) ────────────────────────────────
    //
    // Three tools delegating to `code_explorer_rag::sfd`. The lib enforces path
    ...
    async fn tool_list_sfd_pages(&self, args: &Value) -> Result<Value> {
        ...
        let entry = self.resolve_repo(repo_name)?;
        let repo_path = std::path::Path::new(&entry.path);
        let listing = code_explorer_rag::sfd::list_pages(repo_path);
        let summary = if listing.missing {
            format!(
                "No docs directory at {} — run `code-explorer generate docs` first.",
    ...
    async fn tool_validate_sfd(&self, args: &Value) -> Result<Value> {
        ...
        let entry = self.resolve_repo(repo_name)?;
        let repo_path = std::path::Path::new(&entry.path);
        let report = code_explorer_rag::sfd::validate_draft(repo_path, sub_path).map_err(|e| {
            McpError::InvalidArguments {
            ...
            for page in sorted.iter().take(5) {
                ...
                for iss in page.issues.iter().take(3) {
                    let sev = match iss.severity {
                        code_explorer_rag::validator::Severity::Red => "RED",
                        code_explorer_rag::validator::Severity::Yellow => "YELLOW",
                    };
                    let line = iss
                        .line
                        .map(|n| format!("L{}", n))
...
fn classify_risk(direct: usize, transitive: usize, processes: usize) -> &'static str {
    ...
    // "low" otherwise. Mirrors the upstream Code Explorer risk buckets.
...
mod tests {
    use super::*;
    use code_explorer_core::graph::types::{GraphNode, GraphRelationship, NodeProperties};

    fn test_node(id: &str, label: NodeLabel, name: &str, file_path: &str) -> GraphNode {
        GraphNode {
            id: id.to_string(),
            label,
    ...
    #[test]
    fn test_cached_indexes_refresh_when_snapshot_is_replaced() {
        let storage_dir = std::env::temp_dir().join(format!(
            "code-explorer-mcp-cache-refresh-{}-{}",
            std::process::id(),
        ...
        let snap_path = storage_dir.join("graph.bin");

        let mut first_graph = KnowledgeGraph::new();
        first_graph.add_node(test_node(
        ...
        first_graph.add_node(test_node(
            "Method:src/old.rs:old_method",
            NodeLabel::Method,
            "old_method",
            "src/old.rs",
        ));
        first_graph.add_relationship(test_rel(
            "old-membership",
            "Class:src/old.rs:OldSymbol",
            "Method:src/old.rs:old_method",
            RelationshipType::HasMethod,
        ));
        code_explorer_db::snapshot::save_snapshot(&first_graph, &snap_path).unwrap();

        let mut backend = LocalBackend::new();
        let (cached_graph, cached_indexes, cached_fts) =
            backend.load_cached_indexes(&snap_path).unwrap();
        assert!(cached_graph
            .get_node("Class:src/old.rs:OldSymbol")
            .is_some());
        ...
        replacement_graph.add_node(test_node(
            "Method:src/new.rs:replacement_method_with_longer_name",
            NodeLabel::Method,
            "replacement_method_with_longer_name",
            "src/new.rs",
        ));
        replacement_graph.add_relationship(test_rel(
            "replacement-membership-with-longer-id",
            "Class:src/new.rs:ReplacementSymbolWithLongerName",
            "Method:src/new.rs:replacement_method_with_longer_name",
            RelationshipType::HasMethod,
        ));
        code_explorer_db::snapshot::save_snapshot(&replacement_graph, &snap_path).unwrap();

        let (refreshed_graph, refreshed_indexes, refreshed_fts) =
            backend.load_cached_indexes(&snap_path).unwrap();

        assert!(!Arc::ptr_eq(&cached_graph, &refreshed_graph));
        assert!(!Arc::ptr_eq(&cached_indexes, &refreshed_indexes));
        assert!(!Arc::ptr_eq(&cached_fts, &refreshed_fts));
        assert!(refreshed_graph
            .get_node("Class:src/new.rs:ReplacementSymbolWithLongerName")
            .is_some());
        assert!(refreshed_graph
            .get_node("Class:src/old.rs:OldSymbol")
            .is_none());
        assert!(refreshed_indexes
            .outgoing
            .contains_key("Class:src/new.rs:ReplacementSymbolWithLongerName"));
        assert!(!refreshed_indexes
            .outgoing
            .contains_key("Class:src/old.rs:OldSymbol"));
        assert_eq!(
    ...
    #[test]
    fn test_cached_snapshot_does_not_survive_snapshot_removal() {
        let storage_dir = std::env::temp_dir().join(format!(
            "code-explorer-mcp-cache-removal-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        graph.add_node(test_node(
            "Class:src/cached.rs:CachedSymbol",
            NodeLabel::Class,
            "CachedSymbol",
            "src/cached.rs",
        ));
        code_explorer_db::snapshot::save_snapshot(&graph, &snap_path).unwrap();

        let mut backend = LocalBackend::new();
        backend.load_cached_indexes(&snap_path).unwrap();
        std::fs::remove_file(&snap_path).unwrap();

        assert!(backend.load_cached_snapshot(&snap_path).is_err());
        assert!(!backend.snapshot_cache.contains_key(&snap_path));
        assert!(!backend.indexes_cache.contains_key(&snap_path));
        assert!(!backend.fts_cache.contains_key(&snap_path));

        std::fs::remove_dir_all(storage_dir).unwrap();
    }

    /// Unique sandbox directory for a test, cleaned up by the caller.
    fn sandbox(tag: &str) -> PathBuf {
        let dir = std::env::temp_dir().join(format!(
            "code-explorer-mcp-{tag}-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
    ...
    fn write_registry(path: &std::path::Path, entries: &[RegistryEntry]) {
        std::fs::write(path, serde_json::to_string_pretty(entries).unwrap()).unwrap();
    }

    fn entry_for(path: &std::path::Path) -> RegistryEntry {
        RegistryEntry {
            name: path
                .file_name()
                .and_then(|n| n.to_str())
                ...
                .to_string(),
            path: path.display().to_string(),
            storage_path: path.join(".codeexplorer").display().to_string(),
    ...
    fn write_index_meta(repo: &std::path::Path) {
        let storage = repo.join(".codeexplorer");
        std::fs::create_dir_all(&storage).unwrap();
        let meta = repo_manager::RepoMeta {
            repo_path: repo.display().to_string(),
            last_commit: "cafebabe".to_string(),
            indexed_at: "2026-09-08T01:00:00Z".to_string(),
            schema_version: None,
            stats: None,
        };
        std::fs::write(
    ...
    /// D1: a repository indexed *after* the MCP server started must resolve
    /// without restarting the server.
    #[tokio::test]
    async fn test_registry_reload_after_repo_indexed_mid_session() {
        let dir = sandbox("d1-reload");
        let registry_path = dir.join("registry.json");
        write_registry(&registry_path, &[]);

        let repo = dir.join("indexed-later");
        std::fs::create_dir_all(&repo).unwrap();
        write_index_meta(&repo);
        std::fs::write(repo.join(".codeexplorer/graph.bin"), b"{}").unwrap();

        let mut backend = LocalBackend::with_registry_path(registry_path.clone());
        backend.init().unwrap();
        assert!(backend.resolve_repo(Some("indexed-later")).is_err());

        // `code-explorer analyze` runs here, in another process.
        write_registry(&registry_path, &[entry_for(&repo)]);

        // Any tool call refreshes the registry before dispatching.
        let listed = backend.call_tool("list_repos", &json!({})).await.unwrap();
        assert!(serde_json::to_string(&listed)
            .unwrap()
            .contains("indexed-later"));
        let resolved = backend.resolve_repo(Some("indexed-later")).unwrap();
        assert_eq!(resolved.name, "indexed-later");

        std::fs::remove_dir_all(dir).unwrap();
    }

    /// A registry that becomes unreadable must not drop repositories that were
    /// already resolving.
    #[tokio::test]
    async fn test_unreadable_registry_keeps_previous_snapshot() {
        let dir = sandbox("d1-corrupt");
        let registry_path = dir.join("registry.json");
        let repo = dir.join("still-there");
        std::fs::create_dir_all(&repo).unwrap();
        write_registry(&registry_path, &[entry_for(&repo)]);

        let mut backend = LocalBackend::with_registry_path(registry_path.clone());
        backend.init().unwrap();
        assert!(backend.resolve_repo(Some("still-there")).is_ok());

        std::fs::write(&registry_path, "{ this is not json").unwrap();
        let _ = backend.call_tool("list_repos", &json!({})).await;
        assert!(backend.resolve_repo(Some("still-there")).is_ok());

        std::fs::remove_dir_all(dir).unwrap();
    }

    /// Trailing separators and `.` segments name the same repository.
    #[test]
    fn test_resolve_repo_accepts_non_canonical_path() {
        let dir = sandbox("d1-canonical");
        let repo = dir.join("wt");
        std::fs::create_dir_all(&repo).unwrap();
        let canonical = repo.canonicalize().unwrap();

        let mut backend = LocalBackend::new();
        backend.registry.push(entry_for(&canonical));

        for spelling in [
            repo.display().to_string(),
            format!("{}/", repo.display()),
            format!("{}/./", repo.display()),
            format!("{}/../wt", repo.display()),
        ] {
            let resolved = backend
                .resolve_repo(Some(&spelling))
    ...
    /// An index present on disk but absent from the registry still resolves —
    /// the CLI reads it, so the MCP server must too.
    #[test]
    fn test_resolve_repo_falls_back_to_index_on_disk() {
        let dir = sandbox("d1-orphan");
        let repo = dir.join("orphan-index");
        std::fs::create_dir_all(&repo).unwrap();
        write_index_meta(&repo);

        let backend = LocalBackend::with_registry_path(dir.join("registry.json"));
        let resolved = backend
            .resolve_repo(Some(&repo.display().to_string()))
            .unwrap();

        assert_eq!(resolved.name, "orphan-index");
        assert_eq!(resolved.last_commit, "cafebabe");
        assert!(resolved.storage_path.ends_with(".codeexplorer"));

        std::fs::remove_dir_all(dir).unwrap();
    }

    /// The residual error must name the path that was searched and the exact
    /// command that repairs the situation.
    #[test]
    fn test_repo_not_found_error_is_actionable() {
        let dir = sandbox("d1-message");
        let repo = dir.join("never-indexed");
        std::fs::create_dir_all(&repo).unwrap();
        let registry_path = dir.join("registry.json");
        write_registry(&registry_path, &[]);
        ...
        assert!(message.contains("never-indexed"), "{message}");
        assert!(
            message.contains(&registry_path.display().to_string()),
            "{message}"
        );
        assert!(message.contains("meta.json"), "{message}");
        assert!(message.contains("code-explorer analyze"), "{message}");

        std::fs::remove_dir_all(dir).unwrap();
    }

    /// A registry entry whose index was deleted must not be offered: an agent
    /// would pick it and every tool would then fail on it.
    #[tokio::test]
    async fn test_list_repos_hides_entries_whose_index_is_gone() {
        let dir = sandbox("list-stale");
        let registry_path = dir.join("registry.json");

        let alive = dir.join("alive");
        std::fs::create_dir_all(&alive).unwrap();
        write_index_meta(&alive);
        std::fs::write(alive.join(".codeexplorer/graph.bin"), b"{}").unwrap();

        let dead = dir.join("dead");
        std::fs::create_dir_all(&dead).unwrap();

        write_registry(&registry_path, &[entry_for(&alive), entry_for(&dead)]);
        let mut backend = LocalBackend::with_registry_path(registry_path);
        backend.init().unwrap();

        let listed = backend.call_tool("list_repos", &json!({})).await.unwrap();
        let text = listed["content"][0]["text"].as_str().unwrap();
        assert!(text.contains("alive"), "{text}");
        assert!(!text.contains("\"name\": \"dead\""), "{text}");
        assert_eq!(listed["_meta"]["counts"]["listed"], 1);
        assert_eq!(listed["_meta"]["counts"]["staleEntriesHidden"], 1);
        assert!(listed["_meta"]["note"]
            .as_str()
            .unwrap()
    ...
    /// Deleting an index mid-session must be reflected on the next call, with
    /// no restart.
    #[tokio::test]
    async fn test_deleting_an_index_removes_it_from_the_list_without_a_restart() {
        let dir = sandbox("list-delete");
        let registry_path = dir.join("registry.json");
        let repo = dir.join("ephemeral");
        std::fs::create_dir_all(&repo).unwrap();
        write_index_meta(&repo);
        std::fs::write(repo.join(".codeexplorer/graph.bin"), b"{}").unwrap();
        write_registry(&registry_path, &[entry_for(&repo)]);
        ...
        let mut backend = LocalBackend::with_registry_path(registry_path);
        backend.init().unwrap();
        let before = backend.call_tool("list_repos", &json!({})).await.unwrap();
        assert_eq!(before["_meta"]["counts"]["listed"], 1);

        std::fs::remove_dir_all(repo.join(".codeexplorer")).unwrap();

        let after = backend.call_tool("list_repos", &json!({})).await.unwrap();
        assert_eq!(after["_meta"]["counts"]["listed"], 0);
        assert_eq!(after["_meta"]["counts"]["staleEntriesHidden"], 1);

        std::fs::remove_dir_all(dir).unwrap();
    }

    /// Light load: 50 sequential `context` calls must keep answering and must
    /// not accumulate cached snapshots, indexes or FTS structures. The cache
    /// sizes and the `Arc` strong counts are the deterministic proxy for a
    /// leak — wall-clock memory is not assertable in a unit test.
    #[tokio::test]
    async fn test_fifty_sequential_context_calls_do_not_accumulate_state() {
        let dir = sandbox("load");
        let repo = dir.join("loaded");
        let storage = repo.join(".codeexplorer");
        std::fs::create_dir_all(&storage).unwrap();

        let mut graph = KnowledgeGraph::new();
        graph.add_node(test_node(
            "Function:src/a.rs:hot_symbol",
            NodeLabel::Function,
            "hot_symbol",
            "src/a.rs",
        ));
        graph.add_node(test_node(
            "Function:src/b.rs:caller",
            NodeLabel::Function,
            "caller",
            "src/b.rs",
        ));
        graph.add_relationship(test_rel(
            "r_call",
            "Function:src/b.rs:caller",
            "Function:src/a.rs:hot_symbol",
            RelationshipType::Calls,
        ...
        let mut strong_after_first = None;
        for round in 0..50 {
            let result = backend
                .call_tool("context", &json!({"repo": "loaded", "name": "hot_symbol"}))
                .await
                .unwrap_or_else(|e| panic!("round {round} failed: {e}"));
            let text = serde_json::to_string(&result).unwrap();
            assert!(text.contains("hot_symbol"), "round {round}: {text}");

            // One repository, one snapshot: nothing may accumulate per call.
            assert_eq!(backend.snapshot_cache.len(), 1, "round {round}");
            assert!(backend.indexes_cache.len() <= 1, "round {round}");
            assert!(backend.fts_cache.len() <= 1, "round {round}");

            let snap_path = storage.join("graph.bin");
            let strong = Arc::strong_count(backend.snapshot_cache.get(&snap_path).unwrap());
            match strong_after_first {
                None => strong_after_first = Some(strong),
                Some(expected) => assert_eq!(
                    strong, expected,
                    "round {round}: a reference to the cached snapshot was retained"
                ),
            }
        }
    ...
    fn test_resolve_repo_by_public_id() {
        ...
        let entry = RegistryEntry {
            ...
            path: "D:/Repos/code-explorer".to_string(),
            storage_path: "D:/Repos/code-explorer/.codeexplorer".to_string(),
        ...
        assert_eq!(resolved.name, "code-explorer");
        assert_eq!(resolved.path, "D:/Repos/code-explorer");
    }
    ...
    fn test_resolve_repo_accepts_windows_verbatim_path_equivalent() {
        ...
        backend.registry.push(RegistryEntry {
            ...
            storage_path: r"\\?\C:\repos\sample-repo\.codeexplorer".to_string(),
    ...
    #[tokio::test]
    async fn test_impact_both_traverses_membership_edges_like_cli() {
        let repo_dir = std::env::temp_dir().join(format!(
            "code-explorer-mcp-impact-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        let storage_dir = repo_dir.join(".codeexplorer");
        ...
        code_explorer_db::snapshot::save_snapshot(&graph, &storage_dir.join("graph.bin")).unwrap();

        let mut backend = LocalBackend::new();
        backend.registry.push(RegistryEntry {
            name: "test-impact".to_string(),
        ...
        let callers = rows[0]["callers"].as_array().unwrap();
        let callees = rows[1]["callees"].as_array().unwrap();

        assert!(callers.iter().any(|n| n["name"] == "Agent"));
        assert!(callers.iter().any(|n| n["name"] == "start"));
        assert!(callees.iter().any(|n| n["name"] == "Planner"));
        assert!(callees.iter().any(|n| n["name"] == "createPlan"));
        assert_eq!(result["_meta"]["counts"]["upstream"], 2);
        assert_eq!(result["_meta"]["counts"]["downstream"], 2);
        assert_eq!(result["_meta"]["counts"]["total"], 4);

        std::fs::remove_dir_all(repo_dir).unwrap();
    }

    /// Prose repositories: a Markdown heading must be reachable through the
    /// MCP surface, not only through the CLI.
    #[tokio::test]
    async fn test_search_code_and_context_reach_document_headings() {
        let repo_dir = std::env::temp_dir().join(format!(
            "code-explorer-mcp-prose-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        std::fs::create_dir_all(repo_dir.join("chapters")).unwrap();
        std::fs::write(
            repo_dir.join("chapters/one.md"),
            "# Wandering albatross\n\nbody\n",
        )
        .unwrap();
        ...
        graph.add_node(test_node(
            "Section:chapters/one.md#Wandering albatross",
            NodeLabel::Section,
            "Wandering albatross",
            "chapters/one.md",
        ));
        ...
        let search = backend
            .call_tool(
                "search_code",
                &json!({"repo": "prose-repo", "query": "albatross"}),
            )
            .await
            .unwrap();
        let text = serde_json::to_string(&search).unwrap();
        assert!(text.contains("Wandering albatross"), "{text}");

        let context = backend
            .call_tool(
                "context",
                &json!({"repo": "prose-repo", "name": "Wandering albatross"}),
            )
            .await
            .unwrap();
        let text = serde_json::to_string(&context).unwrap();
        assert!(text.contains("chapters/one.md"), "{text}");

        std::fs::remove_dir_all(repo_dir).unwrap();
    }

    #[test]
    fn test_read_code_snippet_clamps_start_after_file_end() {
        let repo_dir = std::env::temp_dir().join(format!(
            "code-explorer-read-snippet-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .unwrap()
        ...
        std::fs::write(
            repo_dir.join("src").join("sample.rs"),
            "alpha\nbeta\ngamma\n",
        )
        .unwrap();

        let snippet = LocalBackend::read_code_snippet(&repo_dir, "src/sample.rs", Some(20), None);
    ...
    async fn test_cypher_write_rejected() {
        ...
        backend.registry.push(RegistryEntry {
            ...
            storage_path: "/tmp/test/.codeexplorer".to_string(),
            indexed_at: "2024-01-01".to_string(),
            last_commit: "abc123".to_string(),
            stats: None,
        });
