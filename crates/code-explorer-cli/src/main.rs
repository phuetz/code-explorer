...
enum Commands {
    ...
    Analyze {
        ...
        /// Index prose files (Markdown, .txt, .rst) as documents with their
        /// headings as symbols. Default: automatic — on when the repository is
        /// mostly prose (less than 30% code files).
        #[arg(long, conflicts_with = "no_docs")]
        include_docs: bool,
        /// Never index prose files, whatever the repository looks like
        #[arg(long)]
        no_docs: bool,
        /// Extra directory name or `*` glob to keep out of the index.
        /// Repeatable: --exclude vendor --exclude "*.generated"
        #[arg(long = "exclude", value_name = "PATTERN")]
        exclude: Vec<String>,
        /// Re-include a directory an exclusion would have dropped.
        /// Repeatable, and wins over every exclusion.
        #[arg(long = "include", value_name = "PATTERN")]
        include: Vec<String>,
        /// Do not apply the built-in exclusions (node_modules, dist, build,
        /// target, .next, .venv, venv, __pycache__, coverage, _archive,
        /// backup*, *.bak, ...). --exclude still applies.
        #[arg(long)]
        no_default_excludes: bool,
        /// Refuse to index more candidate files than this, instead of running
    ...
    #[command(
        after_help = "Examples:\n  code-explorer doctor\n  code-explorer doctor ../other-checkout\n  code-explorer doctor --json"
    )]
    Doctor {
        /// Path to the repository (defaults to current directory)
        #[arg(default_value = ".")]
    ...
    #[command(
        after_help = "Examples:\n  code-explorer trace-files LetterController\n  code-explorer trace-files CustomerService --depth 3 --json"
    )]
    TraceFiles {
    ...
    #[command(
        after_help = "Examples:\n  code-explorer diagram LetterController --type flowchart\n  code-explorer diagram CustomerService --format slides\n  code-explorer diagram LetterController --type class --output diagram.md"
    )]
    Diagram {
    ...
    #[command(
        after_help = "Examples:\n  code-explorer coverage LetterService\n  code-explorer coverage --json\n  code-explorer coverage LetterService --path D:\\taf\\sample-app"
    )]
    Coverage {
...
/// Windows gives the main thread a 1 MiB stack, and the CLI future (every
/// subcommand's state machine lives in it) outgrew that on 2026-09-09: each
/// invocation died with « thread 'main' has overflowed its stack » before
/// parsing its arguments. Run the real entry point on a thread with a wide
/// stack and keep its future on the heap.
