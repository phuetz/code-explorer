...
pub(super) fn describe_service_fr(name: &str) -> &'static str {
    ...
    if lower.contains("product") {
        "Gestion du catalogue produits et paramétrage"
    } else if lower.contains("price") {
        "Calcul des grilles tarifaires et tranches"
        ...
        "Génération et envoi de lettres"
    ...
    } else if lower.contains("grpprod") {
        "Gestion des groupes de produits"
    } else if lower.contains("appconfig") {
        "Données et paramètres applicatifs"
    } else if lower.contains("background") {
        "Traitement asynchrone (Hangfire)"
    } else if lower.contains("extledger") {
        "Export comptable vers EXTLEDGER"
    } else if lower.contains("numbering") {
