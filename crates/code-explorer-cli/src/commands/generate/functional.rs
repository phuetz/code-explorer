...
pub(super) fn generate_functional_guide(
    ...
    writeln!(f, "### Recherche Client")?;
    writeln!(f)?;
    writeln!(f, "```mermaid")?;
    writeln!(f, "sequenceDiagram")?;
    writeln!(f, "    participant U as Utilisateur")?;
    writeln!(f, "    participant C as CustomerController")?;
    writeln!(f, "    participant S as CustomerService")?;
    writeln!(f, "    participant API as ExternalApi API")?;
    writeln!(f, "    U->>C: Recherche (ID ou Nom)")?;
    writeln!(f, "    C->>S: CustomerLookup(filtre)")?;
    writeln!(f, "    S->>API: AcmeClient.CustomersGetAsync()")?;
    writeln!(f, "    API-->>S: CustomerLite[]")?;
    writeln!(f, "    S->>API: PartnerClient.MembersGetAsync()")?;
    writeln!(f, "    API-->>S: Members (contacts liés)")?;
    writeln!(f, "    S-->>C: Liste clients")?;
    writeln!(f, "    C-->>U: Grille Telerik avec résultats")?;
    writeln!(f, "```")?;
    writeln!(f)?;

    writeln!(f, "### Création Commande")?;
    writeln!(f)?;
    writeln!(f, "```mermaid")?;
    writeln!(f, "sequenceDiagram")?;
    writeln!(f, "    participant U as Utilisateur")?;
    writeln!(f, "    participant C as CasesController")?;
    writeln!(f, "    participant S as CaseService")?;
    writeln!(f, "    participant DB as Entity Framework")?;
    writeln!(f, "    U->>C: Sélection Catégorie + Groupe Produits")?;
    writeln!(f, "    C->>S: AfficherProduits(idGrpProd)")?;
    writeln!(f, "    S-->>C: Liste Produits disponibles")?;
    writeln!(f, "    U->>C: Choix Produits + Dates")?;
    writeln!(f, "    C->>S: CreateCase(CaseRecord)")?;
    writeln!(f, "    S->>S: Calcul Tarifs + Plafonds")?;
    writeln!(f, "    S->>DB: Insert Commande + Lignes")?;
    writeln!(f, "    DB-->>S: OK")?;
    writeln!(f, "    S-->>C: Commande créée")?;
    writeln!(f, "    C-->>U: Page détails commande")?;
    writeln!(f, "```")?;
    writeln!(f)?;

    writeln!(f, "### Export EXTLEDGER")?;
    writeln!(f)?;
    writeln!(f, "```mermaid")?;
    writeln!(f, "sequenceDiagram")?;
    writeln!(f, "    participant U as Utilisateur")?;
    writeln!(f, "    participant C as InvoicesController")?;
    writeln!(f, "    participant S as InvoiceService")?;
    writeln!(f, "    participant DB as Entity Framework")?;
    writeln!(f, "    U->>C: Validation paiements")?;
    writeln!(f, "    C->>S: PaymentValidation()")?;
    writeln!(f, "    S->>S: Vérification montants + plafonds")?;
    writeln!(f, "    S->>DB: Mise à jour statuts")?;
    writeln!(f, "    U->>C: Générer relevé")?;
    writeln!(f, "    C->>S: GenerateStatement()")?;
    writeln!(f, "    S->>DB: Création relevés")?;
    writeln!(f, "    U->>C: Export EXTLEDGER")?;
    writeln!(f, "    C->>S: ExportLedger()")?;
    writeln!(f, "    S->>S: Formatage FluxA")?;
    writeln!(f, "    S-->>C: Fichier EXTLEDGER")?;
...
pub(super) fn describe_project_fr(name: &str) -> &'static str {
    ...
    if lower.contains("ihm") && !lower.contains("test") {
        ...
        "Utilitaires et attributs partagés"
    } else if lower.contains("letter") && !lower.contains("test") {
        "Génération de lettres (mail merge)"
    } else if lower.contains("apiclient") || lower.contains("webapi") {
        "Client API REST ExternalApi (données clients)"
...
pub(super) fn describe_controller_fr(name: &str) -> &'static str {
    ...
    if lower.contains("administration") {
        "Configurer le référentiel produits (catégories, produits, grilles tarifaires, plafonds, majorations, tarifs, pièces jointes). C'est le socle de paramétrage dont dépend toute l'application."
    } else if lower.contains("case") {
        "Gérer le cycle de vie complet des commandes — de la création à la clôture, en passant par le calcul des prix via les grilles tarifaires et la sélection des produits."
    } else if lower.contains("invoice") {
        "Gérer la chaîne financière : facturation fournisseurs, paiement clients, régularisations, validation et export EXTLEDGER vers le grand livre comptable."
    } else if lower.contains("customer") {
        "Rechercher et consulter les profils des clients et contacts issus du WebAPI ExternalApi, puis les lier aux commandes."
    } else if lower.contains("letter") {
        "Générer des lettres personnalisées aux clients — individuellement ou en masse — à partir de modèles avec champs de fusion."
    } else if lower.contains("statistique") {
        "Produire les tableaux de bord et rapports : suivi budgétaire, comptage commandes, analyse paiements, restitutions mensuelles."
    } else if lower.contains("fournisseur") {
        "Gérer le référentiel des fournisseurs et leur association aux commandes."
    } else if lower.contains("utilisateur") {
        "Administrer les comptes utilisateurs, les profils d'habilitation et les droits d'accès par périmètre."
        ...
        "Suivre les interventions terrain liées aux commandes clients."
    } else if lower.contains("commission") {
        "Gérer les commissions de validation des commandes (nationales et locales)."
    } else if lower.contains("mco") {
        "Module de maintien en condition opérationnelle — suivi de la disponibilité et des cas particuliers."
    } else if lower.contains("archiver") {
        "Archiver les commandes clôturées pour libérer l'espace de travail courant."
