...
<body>
    ...
    <div id="chat-panel" class="chat-panel">
      <div class="chat-header">
        <div class="chat-header-title">
          ...
          <span>Code Explorer Assistant</span>
        </div>
      ...
      <div id="chat-messages" class="chat-messages">
        <div class="message system">
          Bonjour ! Je suis l'assistant Code Explorer. Je peux répondre à vos questions sur ce projet en utilisant le graphe de connaissances.
        </div>
      </div>
  ...
  <script>
      ...
      setTimeout(() => {{
        ...
        if (Array.isArray(PROVENANCE)) {{
          ...
          if (provEntry) {{
            ...
            if (ageDays > 30) {{
              content.insertAdjacentHTML('afterbegin', `<div class="stale-warning"><i data-lucide="alert-triangle" style="width:14px;height:14px;"></i> Documentation enrichie il y a ${{Math.floor(ageDays)}} jours. Relancer <code>code-explorer generate html --enrich</code> pour actualiser.</div>`);
            }}
          }}
        }}
    ...
    function buildChatTranscriptMarkdown() {{
      ...
      const lines = [
        '# Conversation Code Explorer Assistant',
        '',
        '- Projet: ' + project,
      ...
      messages.forEach(function(msg) {{
        const role = msg.classList.contains('user') ? 'Vous' : 'Code Explorer';
    ...
    function chatExportFilename(extension) {{
      ...
      return 'code-explorer-' + base + '-' + stamp + '.' + extension;
    }}
    ...
    function printChatTranscript() {{
      ...
      const html = '<!doctype html>' +
        ...
        '<title>' + escapeChatHtml(project) + ' - Conversation Code Explorer</title>' +
        '<style>' +
        ...
        '</style></head><body>' +
        '<header><h1>Conversation Code Explorer Assistant</h1>' +
    ...
    async function sendChatMessage() {{
      ...
      try {{
        // We assume the server is running on the same host if served via 'code-explorer serve'
        const repoName = document.querySelector('header h1').textContent;
        const headers = {{ 'Content-Type': 'application/json' }};
        const token = window.localStorage.getItem('CODE_EXPLORER_HTTP_TOKEN') || window.localStorage.getItem('code-explorer.httpToken');
        ...
        if (response.status === 401) throw new Error('Token HTTP Code Explorer manquant ou invalide.');
        ...
        setChatMessageText(loadingMsg, err?.message || "Désolé, je ne peux pas répondre pour le moment. Assurez-vous que 'code-explorer serve' est en cours d'exécution.");
    ...
    function addMessage(text, role) {{
      ...
      if (role !== 'system') {{
        ...
        meta.textContent = `${{role === 'user' ? 'Vous' : 'Code Explorer'}} · ${{formatChatTimestamp(div.dataset.createdAt)}}`;
...
mod tests {
    ...
    fn generate_html_site_writes_navigable_redacted_fixture() {
        ...
        let docs = repo.join(".codeexplorer").join("docs");
            ...
            r#"{
              ...
              "pages": [
                ...
                {"id":"processes","title":"Processus","children":[
                  {"id":"letter_generation","path":"processes/letter_generation.md","title":"Flux Lettre","icon":"workflow"}
                ]}
              ]
            }"#,
        )
        ...
        assert!(html.contains("Flux Lettre"));
    ...
    fn repair_embedded_internal_links_resolves_relative_and_alias_targets() {
        let mut pages = BTreeMap::from([
            ...
            (
                "modules/data-sampleentities".to_string(),
            ...
            (
                ...
                (
                    ...
                    r##"<a href="#modules/modules/services%23letterservice" data-page="modules/modules/services" data-anchor="letterservice">LetterService</a>
<a href="#processes/modules/views%23details-cshtml" data-page="processes/modules/views" data-anchor="details-cshtml">Details</a>
<a href="#modules/data-entities%23Acme" data-page="modules/data-entities" data-anchor="Acme">Acme</a>
...
<a href="#modules/ctrl-rootcontroller" data-page="modules/ctrl-rootcontroller">RootController</a>"##
        ...
        assert!(html.contains(
            r##"href="#modules/services%23letterservice" data-page="modules/services""##
        ));
        assert!(
            html.contains(r##"href="#modules/views%23details-cshtml" data-page="modules/views""##)
        );
        assert!(html.contains(
            r##"href="#modules/data-sampleentities%23Acme" data-page="modules/data-sampleentities""##
        ));
