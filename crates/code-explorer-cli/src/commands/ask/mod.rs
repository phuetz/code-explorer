...
mod tests {
    ...
    fn tool_repo_is_locked_to_selected_repository() {
        ...
        assert_eq!(args["query"], "TraceLogger");
    }
    ...
    fn searchable_question_terms_keep_code_identifiers_and_drop_french_fillers() {
        let terms = searchable_question_terms(
            "Explique moi l'utilisation de la librairie TraceLogger et LetterGenerate.",
        );

        assert!(terms.contains(&"tracelogger".to_string()));
        assert!(terms.contains(&"lettergenerate".to_string()));
