...
pub async fn get_process_flows(state: State<'_, AppState>) -> Result<Vec<ProcessFlow>, String> {
    ...
    for node in graph.iter_nodes() {
        if node.label != code_explorer_core::graph::types::NodeLabel::Process {
            continue;
        }
            ...
            if let Some(step_node) = graph.get_node(&current_id) {
                ...
                if let Some(outs) = indexes.outgoing.get(&current_id) {
                    for (target_id, rel_type) in outs {
                        if matches!(
                            rel_type,
                            code_explorer_core::graph::types::RelationshipType::Calls
                                | code_explorer_core::graph::types::RelationshipType::CallsAction
...
fn add_synthetic_business_flows(
    ...
    if graph
        .iter_nodes()
        .any(|n| n.properties.name.contains("Letter"))
    {
        flows.push(ProcessFlow {
            id: "biz-lettres".into(),
            name: "Système de Lettres".into(),
            process_type: "business".into(),
            step_count: 5,
            steps: Vec::new(),
            mermaid: r#"sequenceDiagram
    participant U as Utilisateur
    participant C as LetterController
    participant R as BulkLetterRule
    participant G as LetterGenerate
    participant PDF as Aspose.Words
    U->>C: Sélection type + modèle
    C->>R: GetTypeDestinataire()
    R-->>C: Fournisseur/Commande/Client
    U->>C: Sélection + Imprimer
    C->>R: PrepareBulkLetterCreation()
    loop Pour chaque destinataire
        R->>G: GenerateLetterInfo()
        G->>PDF: Mail merge variables EXTLEDGER
        G->>G: Sauver en base
    end
    ...
    if graph
        .iter_nodes()
        .any(|n| n.properties.name.contains("Payment") || n.properties.name.contains("Invoice"))
    {
        flows.push(ProcessFlow {
            id: "biz-paiements".into(),
            name: "Cycle de Paiement (Facture -> EXTLEDGER)".into(),
            process_type: "business".into(),
            step_count: 6,
            steps: Vec::new(),
    ...
    DemGrPrVal --> DemTransmiseLedger : Circuit externe
    DemGrPrVal --> StatementIssued : Circuit interne
    DemTransmiseLedger --> PaiementRegle : Règlement final
    StatementIssued --> PaiementRegle : Règlement final"#
                .into(),
        });
    }
    ...
    if graph
        .iter_nodes()
        .any(|n| n.properties.name.contains("Price"))
    {
        flows.push(ProcessFlow {
            id: "biz-pricing".into(),
            name: "Moteur de Calcul des Tarifs".into(),
            process_type: "business".into(),
            step_count: 4,
            steps: Vec::new(),
            mermaid: r#"flowchart TD
    A["Montant commande"] --> B["Application des remises"]
    B --> C["Montant comparable"]
    C --> D{Match Palier ?}
    D -->|Oui| J["Tarif = TAUX_PALIER"]
    D -->|Non| K["Hors grille / Tarif min"]"#
                .into(),
        });
    }
}
