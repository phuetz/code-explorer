---
name: code-explorer
description: Use Code Explorer as the code-intelligence layer before broad manual search. Use when answering questions about a codebase, finding symbols, tracing callers and callees, measuring the impact of a change, mapping architecture, generating documentation, or setting up the MCP server for an agent.
---


# Code Explorer CLI Skill

Use Code Explorer first for repository understanding, then read the exact files it points to before editing or making claims. If graph output conflicts with the source, trust the source and note the graph issue.
...
```bash
code-explorer ask "how does the pricing calculation work?" --path /path/to/project
...
- Do not use Code Explorer output as the sole authority for code edits; verify the exact source.
...
- In a long session, the repository changes under you: re-run `code-explorer analyze --incremental` after your own commits, and `--force` when stale output is suspected.
