...
```powershell
pwsh scripts\build-kit.ps1 -OutDir "D:\CascadeProjects\code-explorer-kit-v0"

# Optionnel : embarquer un dépôt déjà indexé dans le kit
pwsh scripts\build-kit.ps1 -OutDir "D:\CascadeProjects\code-explorer-kit-v0" -SeedRepo "D:\taf\sample-app"
```
...
code-explorer ask "comment fonctionne le calcul des barèmes ?"
code-explorer ask "quels controllers appellent le WebAPI ExternalApi ?" --path D:\taf\sample-app
```
...
.\target\release\code-explorer.exe analyze D:\taf\MonAppLegacy

# 3. Générer la documentation HTML
.\target\release\code-explorer.exe generate --path D:\taf\MonAppLegacy html

# 4. Ouvrir dans le navigateur
start D:\taf\MonAppLegacy\.codeexplorer\docs\index.html
...
/code-explorer report --path D:\taf\MonProjet
...
```bash
code-explorer ask "comment fonctionne la validation des paiements ?" --path D:\taf\MonProjet
code-explorer generate html --path D:\taf\MonProjet --enrich
```
...
Code Explorer est distribué sous **[PolyForm Noncommercial License 1.0.0](LICENSE)** — libre pour tout usage non commercial (perso, recherche, éducation, évaluation). L'usage commercial nécessite une licence séparée. Pour une licence commerciale ou un partenariat : patrice.huetz@gmail.com · [agile-up.com](https://agile-up.com).
