...
Intelligence de code basée sur un graphe de connaissances pour agents IA. Code Explorer construit un graphe à partir de votre code source et l'expose via [MCP](https://modelcontextprotocol.io/) (Model Context Protocol) pour l'analyse de code assistée par IA.
...
## Pourquoi Code Explorer ? (vs un assistant IA seul)
...
Code Explorer résout ce problème en pré-indexant **l'intégralité** de votre codebase dans un graphe de connaissances.

| | Assistant IA seul | Assistant IA + Code Explorer |
|---|---|---|
...
| **Analyse d'impact** | Impossible sans lire tout le projet | `code-explorer impact handleRequest` → chaîne complète en 1 seconde |
...
- **Interroger le code** — Commande CLI `code-explorer ask "question"` pour du Q&A basé sur le graphe avec réponses en streaming.
...
Code Explorer offre un support approfondi des projets ASP.NET MVC 5 legacy, idéal pour documenter et comprendre des applications d'entreprise complexes.
...
| **Services Externes** | Détection de clients WebAPI (`new AcmeClient(httpClient)`), références WCF, traçage d'appels HTTP |
| **Traçabilité TraceLogger** | Analyse de couverture — identifie les méthodes instrumentées avec `BeginMethodScope()` |
...
```bash
code-explorer analyze D:\chemin\vers\mon-projet-mvc5
code-explorer generate --path D:\chemin\vers\mon-projet-mvc5 html
...
- **Processus Métier** (B1-B5) avec flux de haut niveau pour les Lettres, les Paiements et le Calcul des Tarifs
...
.\code-explorer.cmd chat         # Backend 3010 + chat React 5176
.\code-explorer.cmd chat -ChatPort 5174  # Compatibilite avec un ancien onglet
.\start-desktop.cmd         # UI desktop + Tauri
.\check-code-explorer.cmd        # Lint/tests/build principaux
```
...
Code Explorer peut aussi être assemblé en kit portable Windows pour une clé USB, un
partage réseau ou un dossier de démonstration chez un client :

```powershell
pwsh scripts\build-kit.ps1 -OutDir "D:\projects\code-explorer-kit-v0"

# Optionnel : embarquer un dépôt déjà indexé dans le kit
pwsh scripts\build-kit.ps1 -OutDir "D:\projects\code-explorer-kit-v0" -SeedRepo "D:\samples\sample-app"
```

Le dossier généré contient `code-explorer.exe`, le chat React de production dans
`web/`, un `launch.bat` double-clic, et un dossier local `data/`. Le lanceur
définit `CODE_EXPLORER_HOME=%~dp0data`, donc la configuration globale,
l'authentification, le registre, les modèles et les dépôts indexés portables
...
Créer `~/.codeexplorer/chat-config.json` :
...
Fournisseurs supportés : **Gemini**, **OpenAI**, **Anthropic**, **OpenRouter**, **Ollama** (local, pas de clé API nécessaire) et **ChatGPT** via le flux OAuth de type Codex pour `code-explorer ask` / le chat `code-explorer serve`. `--enrich` reste sur les fournisseurs API-key compatibles OpenAI.

Pour utiliser votre abonnement ChatGPT au lieu d'une clé API avec `code-explorer ask` et le chat web lancé par `code-explorer serve` :

```bash
code-explorer login
```

Puis configurer `~/.codeexplorer/chat-config.json` avec le fournisseur ChatGPT. `api_key` reste volontairement vide : le jeton est chargé depuis la connexion OAuth créée ci-dessus.
...
Les `.docx` générés embarquent désormais un en-tête configurable (nom client + titre du document), un pied de page (texte de marque + pagination automatique `Page X / Y`), et les métadonnées Word `Fichier > Propriétés`. Pour surcharger les défauts, créer `~/.codeexplorer/brand.json` :
...
- **JAUNE — à corriger** : sections H1/H2 contenant moins de 50 mots, pages Service / Controller sans en-tête `§4 Algorithmes` (méthodo Sample v1.1).
...
code-explorer ask "comment fonctionne le calcul des tarifs ?"
code-explorer ask "quels controllers appellent le WebAPI ExternalApi ?" --path D:\samples\sample-app
```
...
code-explorer query "middleware d'authentification"

# Recherche hybride : BM25 + embeddings sémantiques fusionnés via Reciprocal Rank Fusion
code-explorer query "middleware d'authentification" --hybrid

# Ajouter le reranker LLM par-dessus (BM25 ou hybride)
code-explorer query "middleware d'authentification" --hybrid --rerank
...
```bash
code-explorer shell         # REPL interactif avec auto-complétion
code-explorer dashboard     # Dashboard TUI avec navigation dans le graphe
code-explorer watch         # Surveillance & réindexation automatique
```
...
`code-explorer serve` écoute par défaut sur `127.0.0.1`. Si vous le liez à une
interface réseau comme `0.0.0.0`, définissez d'abord `CODE_EXPLORER_HTTP_TOKEN` ;
le serveur refuse les binds non-loopback sans ce jeton bearer.

### Autres commandes

```bash
code-explorer list          # Lister les dépôts indexés avec statistiques
code-explorer status        # Afficher le statut de l'index du dépôt courant
code-explorer clean         # Supprimer l'index
code-explorer clean --all   # Supprimer tous les dépôts indexés
code-explorer report        # Rapport combiné de santé du code (hotspots + couplage + ownership)
code-explorer report --json # Idem, en JSON
```
...
.\target\release\code-explorer.exe analyze D:\samples\MonAppLegacy

# 3. Générer la documentation HTML
.\target\release\code-explorer.exe generate --path D:\samples\MonAppLegacy html

# 4. Ouvrir dans le navigateur
start D:\samples\MonAppLegacy\.codeexplorer\docs\index.html

# 5. Ou lancer l'application desktop pour une exploration interactive
.\target\release\code-explorer-desktop.exe
```

## Application Desktop

L'application desktop Code Explorer est une application Tauri v2 avec un frontend React 19. Elle fournit une interface visuelle pour explorer le graphe de connaissances de votre code et un système de chat intelligent pour l'analyse de code assistée par IA.
...
En mode serveur MCP, Code Explorer expose ces outils :
...
## Installer comme plugin Claude Code (une commande)

Le dépôt est son propre marketplace de plugins. Avec le binaire `code-explorer`
sur le `PATH` et un index construit (`code-explorer analyze .`), cette commande
installe la compétence **et** le serveur MCP :
...
Ou, dans une session : `/plugin` → marketplace `phuetz/code-explorer` → installer
`code-explorer`. La compétence apparaît sous `/code-explorer:code-explorer` et les
outils MCP `code-explorer` (query, impact, context, hotspots, …) dans la liste MCP.
Désinstaller : `claude plugin uninstall code-explorer@code-explorer`.

## Compétences pour Claude Code et Codex

Deux compétences prêtes à copier apprennent à l'agent à *cartographier d'abord,
lire ensuite* : interroger les symboles, mesurer le rayon d'impact d'un
changement et citer des preuves `fichier:ligne` au lieu de grepper à l'aveugle —
sans plugin :
...
À associer à la compétence [lm-resizer](https://github.com/phuetz/lm-resizer) :
cartographier d'abord, puis filtrer les sorties bruyantes.

## Intégration IA : Trois façons d'utiliser Code Explorer avec l'IA

Code Explorer propose trois approches distinctes pour l'intelligence de code assistée par IA, chacune avec ses avantages :

### 1. Skill Claude Code (`/code-explorer`) -- Recommandé
...
/code-explorer report --path D:\samples\MonProjet

# Ou posez la question naturellement — Claude invoque le skill automatiquement :
"Qu'est-ce qui appelle le PaymentService ?"  # → Claude lance code-explorer impact PaymentService
```

Le skill est défini dans `.claude/skills/code-explorer/SKILL.md` et fonctionne directement pour quiconque clone le dépôt. Une version personnelle (globale) peut être installée dans `~/.claude/skills/code-explorer/SKILL.md` pour l'utiliser dans tous vos projets.
...
Appels LLM directs via API compatible OpenAI pour l'enrichissement de la documentation et le Q&A sur le code. Nécessite `~/.codeexplorer/chat-config.json`.

```bash
code-explorer ask "comment fonctionne la validation des paiements ?" --path D:\samples\MonProjet
code-explorer generate html --path D:\samples\MonProjet --enrich
```
...
|---|---|---|---|
| **Fonctionnement** | Claude lit le graphe directement via la CLI | L'agent IA appelle des outils via JSON-RPC | Code Explorer appelle un LLM externe |
| **Modèle IA** | Claude (votre session en cours) | Tout agent compatible MCP | Gemini, OpenAI, Anthropic, Ollama |
| **Configuration** | Zéro (le skill est dans le dépôt) | `code-explorer setup` | Fichier config + clé API |
...
Code Explorer est un moteur d'intelligence de code indépendant et haute performance, écrit en Rust. Il a été initialement inspiré par le projet [Code Explorer](https://github.com/abhigyanpatwari/Code Explorer) créé par [Abhigyan Patwari](https://github.com/abhigyanpatwari) — merci pour la vision d'origine.

Code Explorer est sa propre base de code Rust, centrée sur :
...
Code Explorer est distribué sous la **[Business Source License 1.1](LICENSE)**. Vous pouvez l'auto-héberger et l'utiliser pour un usage interne ou non commercial ; le fournir à des tiers comme service commercial d'intelligence de code n'est pas permis. La licence bascule vers Apache 2.0 le 31/08/2030.
