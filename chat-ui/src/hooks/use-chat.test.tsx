...
function SendProbe({ llm = null }: { llm?: LlmConfigInfo | null }) {
  ...
  return (
    <button type="button" onClick={() => void sendMessage('Trace le flux lettre')}>
      Envoyer
    </button>
  );
}
...
describe('useChat', () => {
  ...
  it('renames an empty default session from the first user message', async () => {
    ...
    await waitFor(() => {
      const session = useChatStore.getState().sessions[0];
      expect(session.title).toBe('Trace le flux lettre');
  ...
  it('adds grounding instructions to the backend request in verified mode', async () => {
    ...
    expect(chatStream.mock.calls[0][1]).toContain('Trace le flux lettre');
    expect(useChatStore.getState().sessions[0].messages[0].content).toBe('Trace le flux lettre');
