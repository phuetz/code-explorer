...
export function ChatExports({ llm }: Props) {
  ...
  const handleHtml = () => {
    ...
    exportPrintableHtml(
      session,
      exportMetadata,
      document.getElementById('code-explorer-chat-export-source')
    );
  };
  ...
  const handlePdf = async () => {
    ...
    try {
      ...
      await exportPdf(session, exportMetadata, document.getElementById('code-explorer-chat-export-source'));
    } catch (e) {
  ...
  return (
    <div className="relative flex items-center gap-1" aria-label="Exports de la conversation">
      ...
      <button
        ...
        className="control-button rounded-md border p-1.5 transition disabled:cursor-not-allowed disabled:opacity-40"
        aria-label={isPdfExporting ? 'Préparation du PDF natif' : 'Exporter la conversation en PDF natif'}
        title={isPdfExporting ? 'Préparation du PDF natif' : 'Exporter en PDF natif'}
      >
      ...
      >
        {exportError ?? (isPdfExporting ? 'Génération du PDF natif…' : '')}
      </span>
    </div>
  );
}
