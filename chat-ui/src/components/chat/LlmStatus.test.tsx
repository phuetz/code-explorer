  ...
  tailnetHosts: vi.fn(async () => ({
    ...
    hosts: [
      ...
      },
      {
        ...
        dnsName: 'workstation-linux.tail2a752c.ts.net',
        os: 'linux',
...
describe('LlmStatus', () => {
  ...
  it('opens detailed LLM settings from the header badge', async () => {
    ...
    expect(await screen.findByRole('button', { name: /choisir tailnet-host ollama/i })).toBeTruthy();
  ...
  it('shows only online tailnet machine cards even when Ollama models are unavailable', async () => {
    ...
    render(<LlmStatus llm={workstationOnlineState} />);

    fireEvent.click(screen.getByRole('button', { name: /configuration llm/i }));

    await waitFor(() => {
      expect(workstationOnlineState.tailnetHosts).toHaveBeenCalled();
    });
    expect(screen.queryByRole('button', { name: /choisir tailnet-host ollama/i })).toBeNull();
    expect(await screen.findByRole('button', { name: /choisir workstation linux ollama/i })).toBeTruthy();
    expect(screen.getByRole('button', { name: /choisir ollama local/i })).toBeTruthy();
  });
  ...
  it('prefers the smallest detected local chat model by default', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /configuration llm/i }));
    fireEvent.click(await screen.findByRole('button', { name: /choisir workstation linux ollama/i }));
  ...
  it('saves the TAILNET-HOST provider as a tailnet Ollama endpoint', async () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: /configuration llm/i }));
    fireEvent.click(await screen.findByRole('button', { name: /choisir tailnet-host ollama/i }));
    await screen.findByRole('combobox', { name: /modèle/i });
    fireEvent.click(screen.getByRole('button', { name: /enregistrer/i }));
