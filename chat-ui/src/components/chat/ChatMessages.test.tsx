...
describe('ChatMessages', () => {
  ...
  it('shows verified and evidence badges on grounded assistant messages', () => {
          ...
          messages: [
            {
              id: 'm1',
              role: 'assistant',
              content: 'Source: Controllers/LetterController.cs:42',
              createdAt: 1774507059000,
              verified: true,
              toolCalls: [
    ...
    render(
      <ChatMessages
        validSourcePaths={new Set(['controllers/lettercontroller.cs'])}
  ...
  it('lists related files, opens them, and exports their contents', async () => {
      ...
      sessions: [
        {
          ...
          messages: [
            ...
            },
            {
              id: 'm2',
              role: 'assistant',
              content: 'Sources\n- `Controllers/LetterController.cs:42`\n- BAL/LetterService.cs:5-8',
              createdAt: 1774507059000,
            },
          ],
        },
    ...
    fireEvent.click(screen.getByText('Controllers/LetterController.cs'));
    ...
    expect(exported).toContain('// contenu Controllers/LetterController.cs');
    expect(exported).toContain('## BAL/LetterService.cs');
  });
  ...
  it('keeps hallucinated source paths visible as warnings but not clickable/exportable', () => {
    ...
    render(
      <ChatMessages
        ...
        validSourcePaths={new Set(['acme.demo.common/common/logger.cs'])}
      />
    );
  ...
  it('holds source actions until repository file validation completes', () => {
          ...
          messages: [
            {
              id: 'm1',
              role: 'assistant',
              content: 'Source candidate: Acme.Sample.Common/Common/TraceLogger.cs:10',
              createdAt: 1774507059000,
            },
          ],
        },
    ...
    expect(screen.queryByRole('button', { name: /TraceLogger\.cs:10/i })).toBeNull();
    expect(screen.getByRole('button', { name: /exporter les fichiers concernés/i }).hasAttribute('disabled')).toBe(true);
  });
});
