...
describe('ProjectSelector', () => {
  ...
  it('auto-selects the first repo without refetching because selection changed', async () => {
    ...
    await waitFor(() => {
      expect(useChatStore.getState().selectedRepo).toBe('sample-app');
      expect(useChatStore.getState().selectedRepoName).toBe('sample-app');
  ...
  it('uses the backend repo id when duplicate names exist', async () => {
    ...
    await waitFor(() => {
      ...
      expect(useChatStore.getState().selectedRepoName).toBe('code-explorer · first');
      expect(screen.getByText('code-explorer · first')).toBeTruthy();
    });
  });

  it('migrates a persisted repo name to its stable backend id', async () => {
    useChatStore.setState({ selectedRepo: 'sample-app' });
    vi.stubGlobal(
      'fetch',
      vi.fn(async (input: RequestInfo | URL) => {
        const url = String(input);
    ...
    await waitFor(() => {
      expect(useChatStore.getState().selectedRepo).toBe('repo_sample');
      expect(useChatStore.getState().selectedRepoName).toBe('sample-app');
      expect(screen.getByText('sample-app')).toBeTruthy();
    });
  });
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: RequestInfo | URL) => {
        ...
        if (url.endsWith('/api/repos')) {
          return jsonResponse({
            repos: [
              { id: 'repo_sample', name: 'sample-app', path: 'D:/Repos/sample-app' },
              { id: 'repo_lettre', name: 'Lettre', path: 'D:/Repos/Lettre' },
  ...
  it('treats an empty repo list as an indexable state, not a backend error', async () => {
    ...
    expect(screen.getByText(/code-explorer\.cmd analyze -Repo/i)).toBeTruthy();
  });
  ...
  it('copies actionable diagnostics when the backend project list fails', async () => {
    ...
    expect(diagnostic).toContain('.\\code-explorer.cmd doctor');
    expect(diagnostic).toContain('.\\code-explorer.cmd chat -RestartBackend');
  });
});
