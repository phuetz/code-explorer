...
export function ProjectSelector() {
  ...
  const copyErrorDiagnostic = useCallback(async () => {
    ...
    const ok = await copyTextToClipboard(
      [
        'Code Explorer - erreur de liste des projets',
        '',
        error,
        '',
        'Commandes utiles:',
            ...
            <div className="min-h-0 overflow-y-auto">
              ...
              {!error && repos.length === 0 && !loading && (
                <div className="px-3 py-3 text-xs text-[var(--text-muted)]">
                  ...
                  <code className="rounded bg-[var(--code-bg-soft)] px-1 text-[var(--code-text)]">
                    .\code-explorer.cmd analyze -Repo &lt;path&gt;
                  </code>{' '}
                  depuis ce dépôt.
                </div>
              )}
              ...
              {filteredRepos.map((repo, index) => {
                ...
                return (
                  ...
                  >
                    ...
                    {typeof repo.stats?.index_duration_ms === 'number' && (
                      <span className="text-[10px] text-[var(--text-muted)]">
                        ⚡ indexed in {(repo.stats.index_duration_ms / 1000).toFixed(1)}s
                      </span>
                    )}
                  </button>
                );
