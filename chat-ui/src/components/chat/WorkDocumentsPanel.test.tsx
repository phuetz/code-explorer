    ...
    questions: [
      {
        ...
        status: 'answered',
        answer: [
          'TraceLogger trace les méthodes métier et relie les exécutions au code.',
          '',
          '## Sources',
...
describe('WorkDocumentsPanel', () => {
  ...
  it('shows source buttons for answered questions and opens the explorer callback', () => {
    ...
    render(
      <WorkDocumentsPanel
        ...
        validSourcePaths={new Set(['acme.sample/flux/question.cs'])}
      ...
      questions: [
        {
          ...
          context: 'Paramétrage Création Groupe de Produits',
        },
      ],
  ...
  it('exports the current work document as a native DOCX payload', async () => {
    ...
    expect(payload.markdown).toContain('Acme.Sample/Logging/TraceLogger.cs');
  ...
  it('exports the current work document as a native PDF payload', async () => {
    ...
    expect(payload.html).toContain('TraceLogger trace les méthodes métier');
    expect(payload.html).toContain('Acme.Sample/Logging/TraceLogger.cs');
    expect(downloadBlobFile).toHaveBeenCalledWith(
  ...
  it('anchors an imported document to the active React chat project when answering', async () => {
    ...
    expect(sendMessage.mock.calls[0][1]).toMatchObject({ repository: 'repo_sample' });
    expect(useChatStore.getState().workDocuments[0]).toMatchObject({
      repo: 'repo_sample',
      repoName: 'sample-app',
    });
