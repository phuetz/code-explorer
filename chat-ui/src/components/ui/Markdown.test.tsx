...
sequenceDiagram
MVC->>BAL: CreateBulkLetter
BAL-->>MVC: PDF fusionné
Étapes détaillées`);

    expect(normalized).toContain('```mermaid\nsequenceDiagram');
    expect(normalized).toContain('MVC->>BAL: CreateBulkLetter');
...
LetterController --> LetterService
Sources`);

    expect(normalized).toContain('```mermaid\nclassDiagram');
    expect(normalized).toContain('class LetterController {');
    expect(normalized).toContain('+PrintBulkLetterList()');
    expect(normalized).toContain('}\nLetterController --> LetterService');
    expect(normalized).toContain('```\nSources');
  });
});
  ...
  it('turns plain source paths into explorer actions', () => {
    ...
    render(
      <Markdown onOpenSourceReference={onOpen}>
        {'Voir Acme.Sample.ihm/Controllers/LetterController.cs:42 pour le controleur.'}
      </Markdown>
    );

    fireEvent.click(screen.getByRole('button', { name: /LetterController\.cs:42/ }));

    expect(onOpen).toHaveBeenCalledWith({
      path: 'Acme.Sample.ihm/Controllers/LetterController.cs',
      startLine: 42,
      endLine: 42,
    });
  });

  it('keeps source paths inert while repository file validation is pending', () => {
    const onOpen = vi.fn();

    render(
      <Markdown onOpenSourceReference={onOpen} sourceValidationState="pending">
        {'Voir Acme.Sample.ihm/Controllers/LetterController.cs:42 pour le controleur.'}
      </Markdown>
    );

    expect(screen.queryByRole('button', { name: /LetterController\.cs:42/ })).toBeNull();
    expect(screen.getByText(/LetterController\.cs:42/).tagName.toLowerCase()).toBe('code');
  });

  it('does not linkify source paths inside code fences', () => {
    const linked = linkifySourceReferences(
      '```text\nAcme.Sample.ihm/Controllers/LetterController.cs:42\n```'
    );
