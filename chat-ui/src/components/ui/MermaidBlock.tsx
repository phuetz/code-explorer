...
import { Check, Code2, Copy, Download, ExternalLink, Loader2, Maximize2, X } from 'lucide-react';
...
export function MermaidBlock({ text }: Props) {
  ...
  const downloadSvg = () => {
    ...
    link.href = url;
    link.download = 'code-explorer-diagram.svg';
  ...
  const detachDiagram = () => {
    if (!svg) return;
    const popup = window.open('', 'code-explorer-mermaid-diagram', 'popup,width=1280,height=900');
    if (!popup) return;
    popup.document.write(buildDetachedDiagramHtml(svg, text));
    ...
    >
      <div className="mermaid-toolbar flex items-center justify-between gap-3 border-b px-3 py-2 text-xs">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            type="button"
            onClick={detachDiagram}
            disabled={!svg}
            className="icon-button rounded p-1.5 disabled:cursor-not-allowed disabled:opacity-40"
            aria-label="Détacher le diagramme Mermaid dans une nouvelle fenêtre"
            title="Détacher"
          >
...
function buildDetachedDiagramHtml(svg: string, source: string): string {
  return `<!doctype html>
<html lang="fr">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Code Explorer - Diagramme Mermaid</title>
  <style>
    :root { color-scheme: light dark; font-family: Inter, ui-sans-serif, system-ui, sans-serif; }
    body { margin: 0; background: #f8fafc; color: #0f172a; }
    header { position: sticky; top: 0; z-index: 1; display: flex; justify-content: space-between; gap: 1rem; padding: .75rem 1rem; border-bottom: 1px solid #cbd5e1; background: rgba(248,250,252,.94); backdrop-filter: blur(8px); }
    main { height: calc(100vh - 52px); overflow: auto; padding: 1rem; }
    .canvas { min-width: 1120px; width: max-content; margin: 0 auto; }
    .canvas svg { width: 100%; height: auto; max-width: none; }
    details { margin-top: 1rem; }
    pre { max-width: 1120px; overflow: auto; padding: .75rem; border: 1px solid #cbd5e1; border-radius: .5rem; background: #0f172a; color: #e2e8f0; }
    @media (prefers-color-scheme: dark) {
      body { background: #020617; color: #e2e8f0; }
      header { border-color: #1e293b; background: rgba(2,6,23,.92); }
      pre { border-color: #334155; }
    }
  </style>
</head>
<body>
  <header>
    <strong>Code Explorer - Diagramme Mermaid détaché</strong>
    <span>Déplace cette fenêtre sur un autre écran ou maximise-la.</span>
  </header>
  <main>
    <div class="canvas">${ensureSvgNamespace(svg).replace(/^<\?xml[^>]*>\s*/, '')}</div>
    <details>
