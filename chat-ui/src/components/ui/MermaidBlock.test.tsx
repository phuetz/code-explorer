...
describe('MermaidBlock', () => {
  ...
  it('uses a lightweight SVG fallback for large flowcharts', async () => {
    const source = [
      'flowchart TD',
      'A[Code metier:<br/>TraceLogger.Info/Error/BeginMethodScope] --> B[Selection utilisateurs]',
      'B --> C[Customers]',
      'B --> D[Dossiers]',
      'B --> E[Fournisseurs]',
      'C --> F[Preparation lettre]',
    ...
    expect(labels.join('')).toContain('TraceLogger.Info/Error/BeginMethodScope');
    expect(container.querySelector('.mermaid-canvas svg title')?.textContent).toContain(
      'Code metier: / TraceLogger.Info/Error/BeginMethodScope'
    );
...
describe('repairMermaidSource', () => {
  it('quotes flowchart labels that contain punctuation known to break Mermaid parsing', () => {
    ...
    expect(repaired).toContain('G --> H["CreateLetter(...,)"]');
  ...
  it('removes empty edge labels emitted by LLMs before rendering', () => {
    const repaired = repairMermaidSource(
      [
        'flowchart TD',
        'B -->|| C["Selection des produits"]',
        'B -->|Non| D["Groupe complet"]',
      ].join('\n')
    );

    expect(repaired).toContain('B --> C["Selection des produits"]');
