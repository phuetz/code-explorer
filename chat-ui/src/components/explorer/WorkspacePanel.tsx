...
export type WorkspaceTab = 'sources' | 'graph';
...
export function WorkspacePanel({
  ...
  const [tab, setTab] = useState<WorkspaceTab>(
    initialTab ?? (initialGraphTarget ? 'graph' : 'sources')
  );
  ...
  const [activeAnalysisSnapshot, setActiveAnalysisSnapshot] = useState<AnalysisSnapshot | null>(
    null
  );
  ...
  const highlightedSourcePathSet = useMemo(
    () =>
      new Set(
        [...highlightedSourcePaths, ...effectiveRelatedSourceGroups.map((group) => group.path)].map(
  ...
  useEffect(() => {
    if (tab !== 'sources' || sourceTarget) return;
    const firstTarget = firstSourceTarget(effectiveRelatedSourceGroups);
    if (!firstTarget) return;
    let alive = true;
    queueMicrotask(() => {
      if (alive) setSourceTarget(firstTarget);
    });
    return () => {
      alive = false;
    };
  }, [effectiveRelatedSourceGroups, sourceTarget, tab]);
  ...
  const saveCurrentAnalysis = useCallback(() => {
    ...
    currentSession,
    effectiveRelatedSourceGroups,
    saveAnalysisSnapshot,
    selectedRepo,
    selectedRepoName,
  ]);

  const openAnalysisSnapshot = useCallback(
    (snapshot: AnalysisSnapshot) => {
      setActiveAnalysisSnapshot(snapshot);
      setSourceTarget(firstSourceTarget(snapshotSourceGroups(snapshot)));
      selectTab('sources');
    },
    [selectTab]
  );
  ...
  return (
    <aside
      className={
        detached
          ? 'flex h-full w-full min-w-0 flex-col bg-neutral-950'
          : 'flex h-full w-[clamp(420px,31vw,680px)] min-w-[420px] max-w-[52vw] flex-col border-l border-neutral-900 bg-neutral-950'
      }
    >
      <header className="flex min-h-12 items-center gap-2 border-b border-neutral-900 px-3">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <div className="min-w-0">
            ...
            <div className="truncate text-xs text-neutral-500">
              {selectedRepoName ?? selectedRepo ?? 'Aucun projet'}
            </div>
          </div>
        </div>
        {onDetach && (
          <button
            ...
            aria-label="Détacher l'explorateur dans une nouvelle fenêtre"
            title="Détacher"
          >
        ...
        <button
          ...
          className="rounded-md border border-neutral-800 p-1.5 text-neutral-400 hover:bg-neutral-900 hover:text-neutral-100"
          aria-label="Basculer l'explorateur en plein écran"
          title="Plein écran"
        >
...
function firstSourceTarget(groups: SourceReferenceGroup[]): SourceTarget | null {
  const firstGroup = groups[0];
  if (!firstGroup) return null;
  const firstReference = firstGroup.references[0];
  return {
    path: firstReference?.path ?? firstGroup.path,
    startLine: firstReference?.startLine,
    endLine: firstReference?.endLine,
  };
}

function sourceTargetsFromGroups(groups: SourceReferenceGroup[]): SourceTarget[] {
  return groups
    .map((group) => firstSourceTarget([group]))
    .filter((target): target is SourceTarget => target !== null);
}
...
function SourceExplorer({
  ...
  const [outlineFocus, setOutlineFocus] = useState<{ path: string; lineNumber: number } | null>(
    null
  );
  ...
  const treeReady = !treeLoading && !treeError;
  const indexedSourcePaths = useMemo(() => collectFilePathSet(treeState.files), [treeState.files]);
  const verifiedRelatedSourceGroups = useMemo(
    () => filterExistingSourceGroups(relatedSourceGroups, indexedSourcePaths, treeReady),
    [indexedSourcePaths, relatedSourceGroups, treeReady]
  );
  const rejectedRelatedSourceGroups = useMemo(
    () => filterMissingSourceGroups(relatedSourceGroups, indexedSourcePaths, treeReady),
    [indexedSourcePaths, relatedSourceGroups, treeReady]
  );
  const verifiedHighlightedSourcePaths = useMemo(
    () => filterExistingPathSet(highlightedSourcePaths, indexedSourcePaths, treeReady),
    [highlightedSourcePaths, indexedSourcePaths, treeReady]
  );
  const activePathMissing =
    !!activePath && treeReady && !indexedSourcePaths.has(normalizePath(activePath));
  const sourcePathToLoad = activePath && treeReady && !activePathMissing ? activePath : null;
  const activeTargetKey = sourcePathToLoad
    ? `${repo}:${sourcePathToLoad}:${activeStartLine ?? ''}:${activeEndLine ?? ''}`
    : null;
  ...
  useEffect(() => {
    if (!sourcePathToLoad || !activeTargetKey) return;
    let alive = true;
    void mcpClient
      .source(repo, sourcePathToLoad, {
  ...
  }, [activeEndLine, activeStartLine, activeTargetKey, repo, sourcePathToLoad]);
  ...
  const relatedNavigationTargets = useMemo(
    () => sourceTargetsFromGroups(verifiedRelatedSourceGroups),
    [verifiedRelatedSourceGroups]
  );
  const activeRelatedIndex = useMemo(() => {
    if (!sourcePathToLoad) return -1;
    return relatedNavigationTargets.findIndex(
      (item) => normalizePath(item.path) === normalizePath(sourcePathToLoad)
    );
  }, [relatedNavigationTargets, sourcePathToLoad]);
  const relatedPositionLabel =
    relatedNavigationTargets.length > 0
      ? `${activeRelatedIndex >= 0 ? activeRelatedIndex + 1 : 0}/${relatedNavigationTargets.length}`
      : '';
  const openRelatedTarget = useCallback(
    (targetToOpen: SourceTarget | null | undefined) => {
      if (!targetToOpen) return;
      openFile(targetToOpen.path, targetToOpen.startLine, targetToOpen.endLine);
    },
    [openFile]
  );
  const openFirstRelatedSource = useCallback(() => {
    openRelatedTarget(relatedNavigationTargets[0]);
  }, [openRelatedTarget, relatedNavigationTargets]);
  const openPreviousRelatedSource = useCallback(() => {
    if (relatedNavigationTargets.length === 0) return;
    const currentIndex = activeRelatedIndex >= 0 ? activeRelatedIndex : 0;
    const previousIndex =
      (currentIndex + relatedNavigationTargets.length - 1) % relatedNavigationTargets.length;
    openRelatedTarget(relatedNavigationTargets[previousIndex]);
  }, [activeRelatedIndex, openRelatedTarget, relatedNavigationTargets]);
  const openNextRelatedSource = useCallback(() => {
    if (relatedNavigationTargets.length === 0) return;
    const currentIndex = activeRelatedIndex >= 0 ? activeRelatedIndex : -1;
    const nextIndex = (currentIndex + 1) % relatedNavigationTargets.length;
    openRelatedTarget(relatedNavigationTargets[nextIndex]);
  }, [activeRelatedIndex, openRelatedTarget, relatedNavigationTargets]);

  useEffect(() => {
    if (!treeReady || sourcePathToLoad || manualTarget || target) return;
    const firstGroup = verifiedRelatedSourceGroups[0];
    const firstReference =
      firstGroup?.references[0] ?? (firstGroup ? { path: firstGroup.path } : null);
    if (!firstReference) return;
    let alive = true;
    queueMicrotask(() => {
      if (alive) openFile(firstReference.path, firstReference.startLine, firstReference.endLine);
    });
    return () => {
      alive = false;
    };
  }, [manualTarget, openFile, sourcePathToLoad, target, treeReady, verifiedRelatedSourceGroups]);

  const relatedFolderCounts = useMemo(
    () => buildRelatedFolderCounts(verifiedRelatedSourceGroups),
    [verifiedRelatedSourceGroups]
  );
  const autoExpandedPaths = useMemo(
    () =>
      buildAutoExpandedFolders(
        treeState.files,
        verifiedRelatedSourceGroups,
        sourcePathToLoad,
        true
      ),
    [sourcePathToLoad, treeState.files, verifiedRelatedSourceGroups]
  );
  ...
  const filteredTree = useMemo(
    () => filterTree(treeState.files, filter),
    [treeState.files, filter]
  );
  const displayedTree = useMemo(
    () =>
      showRelatedOnly
        ? filterTreeToRelated(filteredTree, verifiedHighlightedSourcePaths)
        : sortTreeByRelated(filteredTree, verifiedHighlightedSourcePaths, relatedFolderCounts),
    [filteredTree, relatedFolderCounts, showRelatedOnly, verifiedHighlightedSourcePaths]
  );
  const sourceLoading = !!activeTargetKey && sourceState.key !== activeTargetKey;
  const source = activeTargetKey && !sourceLoading ? sourceState.source : null;
  const sourceError = activePathMissing
    ? `Fichier ignoré: '${activePath}' n'existe pas dans l'index du dépôt sélectionné.`
    : sourceLoading
      ? null
      : sourceState.error;
  const sourceRelatedGroup = useMemo(
    () =>
      verifiedRelatedSourceGroups.find(
        (group) => normalizePath(group.path) === normalizePath(source?.path)
      ),
    [source?.path, verifiedRelatedSourceGroups]
  );
  const sourceMatches = useMemo(
    () => findSourceMatches(source, sourceQuery),
    [source, sourceQuery]
  );
  ...
  const activeOutlineLine =
    source && outlineFocus?.path === source.path ? outlineFocus.lineNumber : null;
  ...
  const askAboutSource = () => {
    if (!source) return;
    const range =
      source.startLine && source.endLine ? ` lignes ${source.startLine}-${source.endLine}` : '';
    setInputDraft(
      `Explique le fichier ${source.path}${range} et ses liens avec le graphe Code Explorer.`
    );
  };
  ...
  const openGraphForSource = async () => {
    ...
    try {
      ...
      if (!symbol) {
        setGraphLookup({
          loading: false,
          error: 'Aucun symbole du graphe trouvé pour ce fichier.',
        });
        return;
      }
      ...
      <div className="grid min-h-0 flex-1 grid-cols-[190px_minmax(0,1fr)]">
        <div className="min-h-0 overflow-auto border-r border-neutral-900 p-2 text-xs">
          <AnalysisSnapshotsPanel
            ...
            canSave={verifiedRelatedSourceGroups.length > 0}
            onSave={onSaveAnalysis}
            onOpen={onOpenAnalysis}
            onClear={onClearAnalysis}
            onDelete={onDeleteAnalysis}
          />
          {(verifiedRelatedSourceGroups.length > 0 || rejectedRelatedSourceGroups.length > 0) && (
            <ExplorerRelatedSourcesPanel
              repo={repo}
              repoLabel={repoLabel}
              groups={verifiedRelatedSourceGroups}
              rejectedGroups={rejectedRelatedSourceGroups}
              activePath={sourcePathToLoad}
              positionLabel={relatedPositionLabel}
              relatedOnly={showRelatedOnly}
              onToggleRelatedOnly={() => setShowRelatedOnly((value) => !value)}
              onOpenReference={(reference) =>
                openFile(reference.path, reference.startLine, reference.endLine)
              }
              onOpenFirst={openFirstRelatedSource}
              onOpenPrevious={openPreviousRelatedSource}
              onOpenNext={openNextRelatedSource}
            />
          )}
          {treeLoading ? (
            ...
            <FileTree
              ...
              highlightedSourcePaths={verifiedHighlightedSourcePaths}
          ...
          {sourceLoading ? (
            ...
            <div className="flex h-full flex-col">
              <div className="border-b border-neutral-900 px-3 py-2 text-xs">
                ...
                {graphLookup.error && (
                  <div className="mt-2 text-[11px] text-red-300">{graphLookup.error}</div>
                )}
                {sourceRelatedGroup && (
...
function AnalysisSnapshotsPanel({
  ...
  return (
    <section
      className="analysis-snapshots mb-2 rounded-lg border p-2"
      aria-label="Analyses sauvegardées"
    >
            ...
            >
              ...
              >
                ...
                <span className="block text-[10px] text-[var(--text-muted)]">
                  ...
                  {snapshot.summary.diagramCount} diagramme
                  {snapshot.summary.diagramCount > 1 ? 's' : ''}
                </span>
              </button>
              <button
                type="button"
...
function ExplorerRelatedSourcesPanel({
  ...
  rejectedGroups: SourceReferenceGroup[];
    ...
    try {
      ...
      downloadTextFile(
        ...
        relatedSourcesMarkdown({
          ...
          repoLabel,
          createdAt: Date.now(),
          title: 'Pack d’analyse Code Explorer',
  ...
  const askAboutRelatedFiles = () => {
    const list = groups
      ...
      .join('\n');
    setInputDraft(
      `Analyse les fichiers concernés par cet échange et explique les liens importants :\n${list}`
    );
  };
  ...
  return (
    <section
      className="related-explorer-panel mb-2 rounded-lg border p-2"
      aria-label="Fichiers concernés dans l'explorateur"
    >
      <div className="mb-2 flex items-center justify-between gap-2">
        <div className="min-w-0">
        ...
        <div className="flex shrink-0 items-center gap-1">
          {groups.length > 0 && (
            <span className="mr-1 rounded bg-[var(--panel-bg-muted)] px-1.5 py-0.5 text-[10px] tabular-nums text-[var(--text-muted)]">
              {positionLabel}
            </span>
          )}
          <button
            ...
            className="icon-button rounded p-1 disabled:cursor-not-allowed disabled:opacity-40"
            aria-label="Ouvrir le premier fichier concerné"
            title="Premier fichier concerné"
          >
            <ChevronUp className="h-3.5 w-3.5" aria-hidden />
          </button>
          <button
            ...
            className="icon-button rounded p-1 disabled:cursor-not-allowed disabled:opacity-40"
            aria-label="Fichier concerné précédent"
          ...
          <button
            ...
            disabled={groups.length < 2}
            className="icon-button rounded p-1 disabled:cursor-not-allowed disabled:opacity-40"
            aria-label="Fichier concerné suivant"
      ...
      {rejectedGroups.length > 0 && (
        <div className="mb-2 rounded-md border border-amber-300/50 bg-amber-50 px-2 py-1.5 text-[10px] text-amber-800 dark:border-amber-500/30 dark:bg-amber-500/10 dark:text-amber-200">
          <div className="flex items-start gap-1.5">
            <AlertTriangle className="mt-0.5 h-3.5 w-3.5 shrink-0" aria-hidden />
            <span>
              {rejectedGroups.length} citation{rejectedGroups.length > 1 ? 's' : ''} ignorée
              {rejectedGroups.length > 1 ? 's' : ''} car absente
              {rejectedGroups.length > 1 ? 's' : ''} de l'index:{' '}
      ...
      {groups.length === 0 ? (
        <div className="rounded-md border border-[var(--border)] px-2 py-1.5 text-[10px] text-[var(--text-muted)]">
          Aucun fichier cité par le modèle n'a été retrouvé dans l'index.
        </div>
      ) : null}
      <div className="space-y-1">
        {visibleGroups.map((group) => {
          const first = group.references[0] ?? { path: group.path };
          const active = normalizePath(group.path) === normalizePath(activePath ?? undefined);
          return (
            <button
              key={group.path}
              type="button"
              ...
              className={`related-explorer-file w-full rounded-md border px-2 py-1 text-left font-mono ${
                active ? 'related-explorer-file-active' : ''
              }`}
      ...
      {groups.length > visibleGroups.length && (
        <div className="mt-1 text-[10px] text-[var(--text-muted)]">
          +{groups.length - visibleGroups.length} autre
        ...
        return (
          <div key={node.path}>
            ...
            >
              ...
              {isRelated && (
                <span
                  className="file-tree-related-dot ml-auto h-1.5 w-1.5 shrink-0 rounded-full"
                  aria-hidden
                />
              )}
            </button>
        ...
        <div className="min-h-0 overflow-auto border-r border-neutral-900 p-2 text-xs">
          {loading && results.length === 0 ? (
            ...
            <div className="p-3 text-neutral-600">
              Lance une recherche pour ouvrir un voisinage graphe.
            </div>
          ) : (
            <div className="space-y-1">
              {results.map((symbol) => (
        ...
        <div className="min-h-0 overflow-auto p-3">
          ...
          {!graph ? (
            ...
            <div className="space-y-3">
              ...
              <GraphMap graph={graph} selectedId={selected?.id ?? null} onSelect={setSelected} />
              {selected && (
                <SelectedNodeDetails
                  node={selected}
                  graph={graph}
...
function buildGraphLayout(nodes: GraphNode[], edges: GraphEdge[]) {
  ...
  const visibleEdges = edges
    ...
    .filter(
      (edge): edge is GraphEdge & { source: PositionedGraphNode; target: PositionedGraphNode } =>
...
function outlineItemFromLine(line: string, lineNumber: number): SourceOutlineItem | null {
  ...
  const typeMatch =
    /^(?:(?:export|public|private|protected|internal|abstract|sealed|static|partial)\s+)*(class|interface|enum|struct|record)\s+([A-Za-z_][A-Za-z0-9_]*)/.exec(
      trimmed
    );
  if (typeMatch) {
  ...
  const rustFunctionMatch =
    /^(?:pub(?:\([^)]*\))?\s+)?(?:async\s+)?fn\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(/.exec(trimmed);
  if (rustFunctionMatch) {
    return { kind: 'method', name: rustFunctionMatch[1], lineNumber, level };
  }

  const propertyMatch =
    /^(?:(?:public|private|protected|internal|static|virtual|override|required|readonly)\s+)+[A-Za-z_][A-Za-z0-9_<>,.?[\]\s]*\s+([A-Za-z_][A-Za-z0-9_]*)\s*\{\s*(?:get|set|init)\b/.exec(
      trimmed
    );
  if (propertyMatch) {
  ...
  const methodMatch =
    /^(?:(?:public|private|protected|internal|static|async|virtual|override|sealed|partial|extern|unsafe)\s+)+[A-Za-z_][A-Za-z0-9_<>,.?[\]\s]*\s+([A-Za-z_][A-Za-z0-9_]*)\s*\([^;]*\)\s*(?:\{|=>|$)/.exec(
      trimmed
    );
  ...
  const arrowFunctionMatch =
    /^(?:export\s+)?(?:const|let|var)\s+([A-Za-z_][A-Za-z0-9_]*)\s*=\s*(?:async\s*)?(?:\([^)]*\)|[A-Za-z_][A-Za-z0-9_]*)\s*=>/.exec(
      trimmed
    );
...
function bestSymbolForSource(
  ...
  return (
    symbols
      ...
      .sort(
        (a, b) =>
          symbolScore(b, sourcePath, sourceName, source) -
          symbolScore(a, sourcePath, sourceName, source)
      )[0] ?? null
  );
}
...
function filterExistingSourceGroups(
  groups: SourceReferenceGroup[],
  indexedSourcePaths: ReadonlySet<string>,
  treeReady: boolean
): SourceReferenceGroup[] {
  if (!treeReady) return groups;
  return groups.filter((group) => indexedSourcePaths.has(normalizePath(group.path)));
}

function filterMissingSourceGroups(
  groups: SourceReferenceGroup[],
  indexedSourcePaths: ReadonlySet<string>,
  treeReady: boolean
): SourceReferenceGroup[] {
...
function filterExistingPathSet(
  paths: ReadonlySet<string>,
  indexedSourcePaths: ReadonlySet<string>,
  treeReady: boolean
): Set<string> {
  if (!treeReady) return new Set(paths);
  const verified = new Set<string>();
  for (const path of paths) {
    const normalized = normalizePath(path);
    if (indexedSourcePaths.has(normalized)) verified.add(normalized);
  }
  return verified;
}
...
function buildAutoExpandedFolders(
  ...
  if (includeTopLevel) {
    nodes
      ...
      .slice(0, 12)
      .forEach((node) => expanded.add(normalizePath(node.path)));
  }
