...
describe('MCPClient errors', () => {
  ...
  it('tests LLM provider configuration without persisting it', async () => {
    ...
    expect(init.method).toBe('POST');
    expect(body).not.toHaveProperty('apiKey');
    expect(body.provider).toBe('tailnet-ollama');
  ...
  it('loads Tailscale host availability for network LLM cards', async () => {
    ...
    expect(result.hosts[0].hostName).toBe('workstation-linux');
  ...
  it('loads source and graph exploration payloads from typed REST endpoints', async () => {
    ...
    const source = await client.source('repo sample', 'src/App.tsx', { start: 1, end: 5 });
    const symbols = await client.symbols('repo sample', 'App');
    const graph = await client.graphNeighborhood('repo sample', 'Method:src/App.tsx:App', 2);
    ...
    expect(String(fetchMock.mock.calls[0][0])).toContain('/api/repos/repo%20sample/source?');
