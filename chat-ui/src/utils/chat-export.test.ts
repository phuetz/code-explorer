...
const session: Session = {
  ...
  messages: [
    ...
    },
    {
      ...
      toolCalls: [
        { id: 't1', name: 'search_code', args: { query: 'lettre' }, status: 'done' },
        { id: 't2', name: 'trace_files', args: { symbol: 'Lettre' }, status: 'error' },
      ],
    },
  ],
};
...
describe('conversationToMarkdown', () => {
  ...
  it('adds a compact source appendix when assistant messages cite files', () => {
    const markdown = conversationToMarkdown(
      {
        ...
        messages: [
          ...session.messages,
          {
            id: 'm3',
            role: 'assistant',
            content: 'Sources: Controllers/LetterController.cs:42 et BAL/LetterService.cs:5-8',
            createdAt: 1774507069000,
          },
        ],
      },
    ...
    expect(markdown).toContain('- Controllers/LetterController.cs:42');
    expect(markdown).toContain('- BAL/LetterService.cs:5-8');
  });
});
...
describe('exportPdf', () => {
  ...
  it('downloads a native PDF when the backend renderer is available', async () => {
    ...
    try {
      ...
      expect(exportSpy.mock.calls[0][0].filename).toMatch(
        /^code-explorer-sample-app-sample-report-\d{8}-\d{6}\.pdf$/
      );
  ...
  it('prepares rendered Mermaid fallbacks and source references for print', async () => {
    ...
    transcript.innerHTML = `
      <section>
        <p><button>Acme.Sample.BAL/Lettre/LetterService.cs:42</button></p>
...
A --> B</code></pre>
    ...
    expect(html).toContain('Acme.Sample.BAL/Lettre/LetterService.cs:42');
  ...
  it('adds cited source files as a printable appendix', async () => {
    ...
    await exportPdf(
      {
        ...
        messages: [
          ...session.messages,
          {
            id: 'm3',
            role: 'assistant',
            content: 'Voir Controllers/LetterController.cs:42.',
            createdAt: 1774507069000,
          },
        ],
      },
    ...
    expect(html).toContain('Controllers/LetterController.cs:42');
  });
...
describe('exportFilename', () => {
  it('normalizes repo and session names', () => {
    ...
    expect(filename).toMatch(/^code-explorer-sample-v2-sample-report-\d{8}-\d{6}\.md$/);
  });

  it('supports standalone HTML exports', () => {
    const filename = exportFilename(session, 'Sample v2', 'html');
    expect(filename).toMatch(/^code-explorer-sample-v2-sample-report-\d{8}-\d{6}\.html$/);
  });
});
