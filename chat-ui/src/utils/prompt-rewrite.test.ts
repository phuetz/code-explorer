...
describe('reformulateChatPrompt', () => {
  ...
  it('turns a draft into a structured, source-focused prompt', () => {
    const rewritten = reformulateChatPrompt('explique TraceLogger', 'sample-app');

    expect(rewritten).toContain('Question à traiter : explique TraceLogger');
  ...
  it('does not wrap a prompt that is already structured', () => {
    const prompt = [
      'Question à traiter : explique TraceLogger',
      '',
      'Réponse attendue :',
      '- Cite les sources.',
    ].join('\n');
