...
describe('workdoc utilities', () => {
    ...
    questions: [
      {
        ...
        answer: [
          ...
          '',
          '## Sources',
          '- Acme.Sample/Logging/TraceLogger.cs',
        ].join('\n'),
  ...
  it('builds a project-aware structured prompt for a question', () => {
    ...
    expect(prompt).toContain('Question à traiter : À quoi sert TraceLogger ?');
  ...
  it('exports answered questions as a final markdown deliverable', () => {
    ...
    expect(markdown).toContain('TraceLogger sert à tracer les méthodes métier');
    expect(markdown).toContain('## Index des sources citées');
    expect(markdown).toContain('| Acme.Sample/Logging/TraceLogger.cs | Q1.1 | 1 |');
  ...
  it('collects a source index for the final review', () => {
    expect(collectWorkDocumentSourceReferences(document)).toEqual([
      {
        path: 'Acme.Sample/Logging/TraceLogger.cs',
        count: 1,
        questionLabels: ['Q1.1'],
      },
    ]);
  ...
  it('generates a safe markdown export filename', () => {
    expect(workDocumentExportFilename(document)).toMatch(
      /^code-explorer-document-travail-questions-sample-sample-app-\d{8}-\d{6}\.md$/
    );
  ...
  it('builds printable HTML for native PDF export', () => {
    ...
    expect(html).toContain('TraceLogger sert à tracer les méthodes métier');
  });
});
