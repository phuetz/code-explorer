...
<a href="docs/infographic-code-explorer.webp"><img src="docs/infographic-code-explorer.webp" width="900" alt="Code Explorer: your repository is parsed with tree-sitter into a knowledge graph of files, symbols, calls, dependencies and changes, queried by developers and AI agents over MCP; impact analysis and 14 languages"/></a>
...
Or, inside a session: `/plugin` → marketplace `phuetz/code-explorer` → install
`code-explorer`. The skill is then available as `/code-explorer:code-explorer`
and the `code-explorer` MCP tools (query, impact, context, hotspots, …) appear
in the MCP list. Remove with `claude plugin uninstall code-explorer@code-explorer`.

## Skills for Claude Code and Codex

Two drop-in skills teach an agent to *map first, then read*: query symbols,
measure the blast radius of a change, and cite `file:line` evidence instead of
...
Pair it with the [lm-resizer](https://github.com/phuetz/lm-resizer) skill:
map first, then filter the noisy output.
...
<p align="center">
  <a href="docs/infographic-ai-engineering-stack.webp"><img src="docs/infographic-ai-engineering-stack.webp" width="900" alt="The AI engineering stack: Code Explorer understands the repository, lm-resizer protects the context budget, Code Buddy 2 acts with AI agents"/></a>
</p>

Code Explorer is the map. [lm-resizer](https://github.com/phuetz/lm-resizer) filters noisy command output before it reaches the model, and [Code Buddy](https://github.com/phuetz/code-buddy) is the agent that acts. Understand, compress, act.

## License

Code Explorer is released under the **[Business Source License 1.1](LICENSE)**. You may
self-host and use it for internal and non-commercial purposes; providing it as a commercial
